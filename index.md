...
- [[toggle_free_flash_loop]] — **토글-프리 dead-time 반복 루프(board-common)**: SW1 토글 없이 빌드→flash→전원사이클 루프가 서는가. **굽기 절반 ✓ 확정**(§= [[jtag_flash_harness]] §7) / **부팅 절반 ? 미확정** — 2026-06-12 SW1=`1,1,1,1` 전원사이클 후 PWM 4ch 무토글 관측, (A)부팅실패 vs (B)측정배선(GND/프로브) 미진단. 다음: UART 텔레메트리(독립 부팅 증거)+GND 확인. warm-reset chip-state·근본원인 미확정(추론 금지)
