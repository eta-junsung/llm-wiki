...
- [[toggle_free_flash_loop]] — **토글-프리 dead-time 반복 루프(board-common)**: SW1 토글 없이 빌드→flash→전원사이클 루프가 서는가. **굽기 절반 ✓ 확정**([[jtag_flash_harness]] §7) / **부팅 절반 ✗ 실패 확정(2026-06-12)** — COM4(UART0 콘솔) `'C'` XMODEM ping 반복·VCC 완전 제거 확인 → ROM→SBL 로드 단계 실패. §③: boot 경로 silicon상 정당·>128Mb flash warm-reset 주소 불일치(full POR만 해소, TRM §5.4.1:530). 잔여 블로커: flash 프로그래밍/설정([[ospi_boot_console_diagnostic]])
...
- [[ospi_boot_console_diagnostic]] — **OSPI 부팅 진단 정본(8kw, 2026-06-12)**: COM4=UART0 콘솔(XDS110, VCC 독립)·boot stage triage recipe(banner별 ROM/SBL/app 가름)·flash 프로그래밍 잔여 블로커(cell 영속성·QE bit·readback XIP 한계)·SBL provenance 확정(byte-identical SDK 프리빌트)·route② mcelf standalone 부팅 미실증
