...
- [[fod_i_coil_observation]] — **FOD I_COIL_SEN 1차 관찰 (branch `feature/adc-noise-fft`, commit `620e62b`까지)**: 이물감지 1차 단계. 신호체인 — I_COIL_SEN(J3.28)=CT(T1 7mV/A)→CR3~6 정류→C55 RC평활된 **DC 포락선**(85kHz는 CT 1차측), 이물→DC 레벨 시프트가 FOD 신호. **2026-07-01 갱신 — §3.5 결정**: Keysight CSV export로 J3.28 노이즈 2회 실측했으나 **둘 다 진폭이 물리적으로 불가능**(pk-pk 20~36V, 노드는 0~3.3V) → 원인 미규명(빈자리), **트랙 A(외부 스코프) 보류** → **온타깃 raw ADC 캡처(버스트+1회 UART 덤프)로 전환**(§4, ⚠️ 아직 미구현). CSV export 시 Fs=N/타임베이스 방법론은 [[instruments]]로 승격. 노이즈 판정 로직은 [[adc_noise_fft_probe]] 공유
...
- [[io_iir_lpf]] — **Io 2차 IIR 저역통과 필터 — 스펙 ingest + 구현 완료(2026-07-01, branch feature/adc-noise-fft `d4542c9`)**. Direct Form I 2차 biquad LP(Butterworth 근사), Io(I_COIL_SEN) ADC 85kHz 스위칭 노이즈 제거. 차분식 `out0=b0·in0+b1·in1+b2·in2+a1·out1+a2·out2`(⚠️a1/a2 부호 접힘=더함), 85kHz 코드계수 b0=b2=9.75e-6·b1=1.95e-5·a1=1.987473·a2=−0.987512(Excel 40kHz값과 구분). **구현**: ALG `eta_alg_iir_lpf`+App `−Vadc/2` offset 파라미터화(`g_i_coil_offset`, 초기값 0), R5F FPU→float, C2000/CLA 포팅, 빌드 경고 0. A6(SW 이동평균) 코드 부재로 대체 대상 없음 — A5 리피터 버스트(N=16) 위에 신규 적층. **실보드 검증은 미실행 — ADC 안정화 트랙 대기**(offset 최종값도 그때 확정). 원본 `raw/io_iir_lpf/`(spec.html·sim.xlsx·impl_16bit.pdf)
