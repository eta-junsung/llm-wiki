...
- [[fod_i_coil_observation]] — **FOD I_COIL_SEN 1차 관찰 (2026-07-01, branch `feature/adc-noise-fft`, 측정 전 절차)**: 이물감지 1차 단계. ★신호체인 정정 — I_COIL_SEN(J3.28)=CT(T1 7mV/A)→CR3~6 정류→C55 RC평활된 **DC 포락선**(85kHz는 CT 1차측), 이물→DC 레벨 시프트가 FOD 신호. 2트랙(스코프 DC/AC/FFT/시간축 + GUI 10Hz ch4 CSV) 동시 캡처 → 검출성 `Δ/σ`. 마커 `ETA_BSP_ADC_DBG_MARK_IDX 0U`. OC 임계 이하 유지(I_COIL_OC 동일 노드). 풀레이트 디지털(버스트 캡처+UART 덤프)은 미룸. 노이즈 판정 [[adc_noise_fft_probe]] 공유
