...
- [[toggle_free_flash_loop]] — **토글-프리 dead-time 반복 루프(board-common)**: SW1 토글 없이 빌드→flash→전원사이클 루프가 서는가. **굽기 절반 ✓ 확정**(§= [[jtag_flash_harness]] §7) / **부팅 절반 ? 미확정** — 2026-06-12 SW1=`1,1,1,1` 전원사이클 후 PWM 4ch 무토글 관측, (A)부팅실패 vs (B)측정배선(GND/프로브) 미진단. **§③(2026-06-12 TRM/UG 조사): boot 경로는 silicon상 정당(BOOTMODE 매 POR 재샘플·OSPI non-XIP) / warm-reset chip-state 근본원인 확정 — >128Mb flash는 warm reset 시 3-byte(ROM)·4-byte(flash) 주소 불일치, full POR만 해소(TRM §5.4.1:530) ⟹ 루프 reset은 진짜 VCC 제거여야 함, SW2 PORz·SW3 RESETz 버튼 어느 것도 flash를 POR 못 함([[CLAUDE]] "리셋/푸시버튼")**. 다음: 그 전원사이클이 (a)전원차단/(b)SW2/(c)SW3 중 무엇이었는지 확정 + UART 텔레메트리 + GND 확인
