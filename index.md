...
- [[wiki_sync_protocol]] — **wiki↔repo 동기화 프로토콜(운영 규약)**: 코드 진실=git / 의도·지식 진실=wiki, 의존 한 방향(repo⟸wiki, 코드 세션은 wiki 모름). **앵커**(status.md의 흡수 커밋 마커)+델타(`git log <앵커>..HEAD`)로 복붙 없이 대조 — 토큰 비용이 파일 크기 무관. 지식 채널=repo `docs/log.md`+토픽노트(lazy-create, 코드측이 채움 — 누가 쓰는지는 wiki 범위 밖). 결정 흐름(bottom-up 승격/top-down 작성)→`decisions/`. 왜는 [[0001-repo-wiki-operating-model]]
...
- [[firmware_naming_conventions]] — **펌웨어 네이밍 컨벤션 표준(전사 공통)**: MISRA-필수 하드룰 + 현대 임베디드 합의(snake·모듈접두사 `eta_`·SCREAMING) 하이브리드. **앵커**=BARR-C 헝가리안 의식적 미채택(`p_`/`b_` 드롭, "MISRA≠헝가리안")·`g_`/`s_` 스코프큐만 유지·`_t` 유지(POSIX 편차 D1). **하드룰**=외부 31자 유일(5.1)·예약식별자 금지(21.x)·stdlib/SDK 비충돌. **편차원장** D1(`_t`/DCL37-C)·D2(SDK 미개명)·D3(루프변수 `i`/`j`). §4 레이어 토큰 결정2(공개=모듈만/내부=레이어포함, 31자 예산 복무). §6 팀수렴(g de-facto=정본·c-nRF52 정렬·c-STM32 grandfather) + 인증 전향(GA→IEC 61508/VA→ISO 26262). g-8kw de-facto 추출 + BARR-C:2018/MISRA C:2012/Linux·Zephyr 대조검증. 짝=[[firmware_git_workflow]], 레이어=[[firmware_layering]]. **전사 skill `eta-firmware-conventions` 플러그인 배포(2026-07-01, eta/eta-ai-tools/) — 이 페이지가 정본, skill은 추출본**. **2026-07-01 갱신**(c팀 oled_tv 02_rx_esb/_shared 리뷰 환원): §2 R1 설계원칙(접두사·완전어화는 31자 예산과 함께, `ETA_PKT_TX_ST_BIT_VOUT_SP`·`comm` 유지 사례) · §4 레이어 토큰 리트머스 명시화(`clock` 예시 추가) · §1 화이트리스트에 `spi` 추가 + 확장판단기준 신설(범용 주변장치 vs 벤더 도메인어 지역화, `esb`/`xfer`는 미결)
- [[firmware_layering]] — **펌웨어 레이어드 아키텍처 표준(전사 공통)**: BSP/HAL/ALG/App 4레이어 + 의존 단방향(App→{BSP,HAL,ALG})·**ALG는 HW 모름**(하드룰 R1~R3). 목적=테스트성(ALG host 실행)·이식성(MCU 교체 시 BSP/HAL만)·SDK 경계 격리. §3 배치규칙(HW종속 산술=HAL/BSP·순수계산=ALG, "PC서 도나?" 리트머스)·§4 **4개 강제 아님**(ALG 비어도·BSP+HAL 병합 2레이어도 준수, 불변식만). §6 팀적용(g-8kw 풀4레이어=레퍼런스·c-nRF52 app+driver 2층·c-STM32 부분). g-8kw de-facto 일반화. 구체적용=[[firmware_layering_8kw]], 네이밍=[[firmware_naming_conventions]]. **전사 skill `eta-firmware-conventions` 플러그인 배포(2026-07-01, eta/eta-ai-tools/) — 이 페이지가 정본, skill은 추출본**. **2026-07-01 갱신**: §3.1 계약/공유 헤더 위생 리트머스 신설("헤더가 host에서도 서나?" — 계약 헤더는 `<stdint.h>`만으로, `stdio.h`/보드 핀은 레이어 누출)
...
- [[am263p_adc_instance_allocation]] — **AM263P ADC 인스턴스/채널 배치 설계 규칙 정본**: 멀티 인스턴스는 표준(비안정 아님). 각 인스턴스=독립 SAR(자체 S/H·시퀀서·결과레지스터·ADCINT). 한 인스턴스 다중 SOC=직렬(스큐+변환시간합 예산, 마지막 EOC coherent read), 다른 인스턴스=병렬(동시 샘플). 상관·고속(V·I 쌍·전류)은 인스턴스 분산+공통 트리거, 무상관·저속(온도)은 한 인스턴스에 몰아 ISR 절약. 인스턴스↑=+ISR/+int_xbar/+flag 비용. 유일한 실제 instability=변환시간합>트리거주기(overflow). **변환시간 예산 & 리피터 N 상한(2026-06-29 정밀 갱신)**: 1회 직렬 cadence≈285 ns(tSH 80 ns[ACQPS=15] + tEOC 205 ns, TRM Table 7-123 PRESCALE=6), 주기당 ~41 변환·리피터 N 상한 ~41. 종전 315 ns·~37은 보수치로 정정. N=64 기각≠리피터 기각(저-N 살아있음). **현 8kw 채택=리피터 버스트 N=16**(종전 "PPB 누적/N=64" stale 정정). **2026-06-30 첫 라이브 실측**: N=16 버스트 cadence ≤195 ns(상한)<정적 285 ns, 트리거→버스트끝 3.12 µs(26.5%)([[adc_repeater_burst_timing]]). △미확정: IRQ 지연 분해·레지스터 readback·동시성 실측
...
- [[am263p_adc_rti_trigger]] — **AM263P ADC SOC 트리거 정본(RTI·EPWM)**: RTI 타이머→ADC SOC 결선 함정(`enableIntr0` 미설정 시 INT0 export 게이트 차단) + JTAG/RAM 측정 시점 함정(reset 없이 read=전부 0 오진) + 검증된 패턴(트리거+ISR-flag). **§5 EPWM0_SOCA 경로(2026-06-26)**: 단일 EPWM SOC fan-out(`ADC_TRIGGER_EPWM0_SOCA=0x08`), ★RTI `enableIntr0` 함정 EPWM엔 적용 안 됨(`ETSEL.SOCAEN` 직접 출력), ★실효 85 kHz=런타임 TBPRD override(syscfg 정적 1000=100 kHz 아님), ADC1 SOC0+SOC1 lockstep 확정. PPB는 [[am263p_adc_ppb_averaging]]로 분리. (파일명은 역사적, 개명 보류)
- [[am263p_adc_repeater_burst]] — **AM263P ADC 트리거 리피터 버스트 정본(2026-06-29 신설)**: 한 트리거에서 SOC를 N회 백투백 버스트→PPB 블록평균→OSINT 매 트리거(출력 85 kHz/인스턴스 유지, 누적과 대비). SDK API 인용(`ADC_configureRepeater` adc.c:101–103, `ADC_RepeaterConfig` adc.h:797–802, `ADC_REPMODE_OVERSAMPLING=0x0`, `repCount=N−1`≤127, `ADC_TRIGGER_REPEATER1/2=0x7E/7F`, REPINST 2개/인스턴스). ⚠️**SDK 헤더 결함**: `ADC_configureRepeater` 프로토타입이 adc.h에 없음→로컬 extern 우회(8kw `eta_bsp_adc.c:43`). 버스트 N은 변환예산에 묶임(N≤~41). HW 버스트 vs SW 이동평균 직교성. 8kw A5 N=16 전환(PR#11 main `2c4ff85`). **§3 라이브 실측(2026-06-30)**: 트리거당 OSINT 1회=85 kHz·버스트 3.12 µs([[adc_repeater_burst_timing]])
- [[am263p_edma_adc_offload]] — **AM263P EDMA의 ADC 적재 적합성 분석(2026-06-29 신설, 결정 보류)**: ADC→EDMA HW 근거(TDMAEN로 DMA read를 EOC 래치 후 정렬 ch07_5:2120, 결과레지스터 버스컨트롤러별 복제 무경합 :2467, EDMA 64채널). ★빈자리 해소: TRM엔 ADC가 DMA 트리거 소스 표에 **없으나**(ch04 ADC DMA Requests 표 부재), **SDK는 ADC INT를 dma_xbar 소스로 제공**(`dma_xbar_am263px.syscfg.js:69–93`)→ADC INT→DMA 크로스바→EDMA 가능. 분석: NoRTOS 단일코어 모니터링에선 EDMA 실익=ISR 폭증 회피·burst 원자전송 둘뿐(85 kHz 실적재 전제), 캐시는 비용. 미실증·채택 보류
- [[am263p_adc_ppb_averaging]] — **AM263P ControlSS ADC PPB HW 오버샘플 평균 정본**: PPB가 CPU 없이 오버샘플 평균 자동 계산, **2의 거듭제곱(≤1024)일 때만**(평균=우측 비트시프트 `SHIFT=n`→÷2^n). 변환마다 PSUM/PCOUNT 누적→limit 도달 시 SUM load+OSINT. ISR을 EOC 아닌 OSINT(평균완료)에 건다(8kw N=64·85kHz⇒1.33kHz). SDK v2 API(`ADC_setupPPB`/`setPPBCountLimit`/`setPPBShiftValue`/`readPPBSum`, shift≤10). ★`selectPPBOSINTSource`는 라우팅 아니라 OSINTSEL 이벤트(PCount vs PCount/Sync) 선택. lockstep으로 한 트리거 다중 SOC 동시 누적. 8kw 적용 main PR #6 `d673e74`(N=64)→PR#11 리피터 버스트 N=16 전환. §6 "트리거당 N회" 노하우 **2026-06-30 실측 충족**(OSINT=85 kHz 트리거당 1회, ADC0; [[adc_repeater_burst_timing]])
...
- [[firmware_layering_8kw]] — **g-8kw 레이어드 아키텍처 적용 (BSP/HAL/ALG/App)**: 전사 표준 [[firmware_layering]]의 구체 적용. PR #5 전면 재구성(동작 불변, 2026-06-25). src/{bsp,hal,alg,app}/ 모듈 인스턴스·빈 경계(eta_alg_control·eta_hal_pwm는 A3/P3 대기)·검증 기록. gui.py:74 비자명 결합 경고(ETA_BSP_PWM_DEADTIME_NS regex 직접 편집). 레이어 정의·불변식·배치규칙은 표준 위임
...
- [[adc_noise_fft_probe]] — **ADC 노이즈 스코프 FFT 진단 + N vs 트리거 위상 결정 (2026-06-30, 측정 전 프로브 계획)**: 백색→N↑ / 스위칭 상관(85kHz 고조파 피크)→트리거 위상. ★근거=ADC 트리거가 EPWM에 위상고정 → 스위칭 상관 노이즈는 coherent → √N 평균 무효(위상이 유일 레버). 프로브=GA_Vin(J3.26)·GA_Iin(J3.29) DC핀이 노이즈 판별 최적, I_COIL(J3.28)은 정류·평활 DC 포락선이라 순수 노이즈엔 리플 상재하나 FOD 프로브로는 정본. 마커=HS1게이트(J4.39)+디버그GPIO95(J4.31 OSINT 토글). MSOX3104T Hanning/0~500kHz/RMS평균·짧은GND. 버스트 N=16=4.56µs≈주기39% → 위상+저N 상호작용. 회로도 [[board_schematic_v1_0e00]]. **2026-07-01 주의 추가**: 인접 FOD 트랙에서 같은 스코프로 I_COIL_SEN 진폭 이상 재현(아래) — GA_Vin/GA_Iin 측정 전 검산 권고
- [[fod_i_coil_observation]] — **FOD I_COIL_SEN 1차 관찰 (main 병합 `953d4f3` PR #12, commit `620e62b`까지)**: 이물감지 1차 단계. 신호체인 — I_COIL_SEN(J3.28)=CT(T1 7mV/A)→CR3~6 정류→C55 RC평활된 **DC 포락선**(85kHz는 CT 1차측), 이물→DC 레벨 시프트가 FOD 신호. **§3.5 결정**: Keysight CSV export로 J3.28 노이즈 2회 실측했으나 **둘 다 진폭이 물리적으로 불가능**(pk-pk 20~36V, 노드는 0~3.3V) → 원인 미규명(빈자리), **트랙 A(외부 스코프) 보류** → **온타깃 raw ADC 캡처(버스트+1회 UART 덤프)로 전환**(§4). CSV export 시 Fs=N/타임베이스 방법론은 [[instruments]]로 승격. **2026-07-01 갱신**: 후속 스펙이 branch `feature/adc-raw-capture`(`docs/adc_raw_capture.md`)에 작성됨(버퍼 2048~4096샘플 후보·GA_Vin/GA_Iin 확장 권고) — **구현은 아직 착수 전**. 노이즈 판정 로직은 [[adc_noise_fft_probe]] 공유
...
- [[pc_monitor_gui]] — **host PC GUI**(`tools/gui/gui.py`, 2026-06-25 main 재확인): ADC 텔레메트리 모니터(18B 수신·6ch 표·플롯·CSV) + **dead-time 빌드/플래시 컨트롤**(Spinbox→write→build→flash, **100~500 ns**, headless CLI `--deadtime N --build --flash`). flash=`tools/ospi_flash/run_flash_node_8kw.{ps1,sh}`(OS 분기)·gmake 3순위 탐색(config.mk→PATH→C:\ti). **PyInstaller exe는 미구현**(.spec·dist·exe 부재, frozen 분기만 intent-trace) → 배포는 py+런처 확정. 런치 구조·결정은 [[gui_launch_architecture]]
- [[gui_launch_architecture]] — **8kw GUI 런치 구조 + "현행 유지" 결정**: OS 무관 `tools/gui/launch.py`(venv 자동생성·requirements 설치·repo루트 cwd) + thin shim 2개(`run-gui-linux.sh`·`run-gui-windows.bat`, `316e649`)가 종전 `gui.bat`/`launch_gui.ps1` 대체. deadtime=소스수정→CCS빌드→JTAG flash라 **개발등급 환경 의존** → 브라우저·exe·단일명령 통합 **불채택**(제로설치 청중 부재·단일 아티팩트 번들 불가·OS 양쪽 더블클릭 단일파일 부재). lint: README가 launch.py 미언급·`ubuntu` 브랜치 오기
...
- [[it6600c_wifi_gui]] — **IT6600C 양방향 전원공급장치 WiFi 연동 PC GUI**(이슈 #9, 당장 착수 예정 없음): SCPI over LAN/WiFi로 실시간 전압·전류·전력 수신·표시·CSV 로깅. 권장=Python+pyvisa+Tkinter([[pc_monitor_gui]] 선례 스택). 미결=VISA 주소·SCPI 명령셋·샘플링 주기·`tools/` 하위 위치
...
- [[io_iir_lpf]] — **Io 2차 IIR 저역통과 필터 — 스펙 ingest + 구현 완료(2026-07-01, commit `d4542c9`, main 병합 PR #12 `953d4f3`)**. Direct Form I 2차 biquad LP(Butterworth 근사), Io(I_COIL_SEN) ADC 85kHz 스위칭 노이즈 제거. 차분식 `out0=b0·in0+b1·in1+b2·in2+a1·out1+a2·out2`(⚠️a1/a2 부호 접힘=더함), 85kHz 코드계수 b0=b2=9.75e-6·b1=1.95e-5·a1=1.987473·a2=−0.987512(Excel 40kHz값과 구분). **구현**: ALG `eta_alg_iir_lpf`+App `−Vadc/2` offset 파라미터화(`g_i_coil_offset`, 초기값 0), R5F FPU→float, C2000/CLA 포팅, 빌드 경고 0. A6(SW 이동평균) 코드 부재로 대체 대상 없음 — A5 리피터 버스트(N=16) 위에 신규 적층. **실보드 검증은 미실행 — ADC 안정화 트랙 대기**(offset 최종값도 그때 확정). 원본 `raw/io_iir_lpf/`(spec.html·sim.xlsx·impl_16bit.pdf)
