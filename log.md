...
- **신설 `weekly_report_2026-06-24_30.md`**: ① 6/24 확정 작업 처리(GUI 소수점 3자리·트리거 전환+HW평균 완료) ② ADC 트리거 RTI→EPWM0_SOCA + PPB N=64(PR #6) ③ PPB 누적→리피터 버스트 블록평균 N=16 전환(PR #11, 출력 85kHz 복귀) + Saleae 실측(85.03kHz·트리거당 OSINT 1회) + 변환예산 N상한~41 ④ 8kW 회로도 센서 신호체인 분석 + FFT 측정계획 ⑤ FOD 1차 관찰 착수(`6993a40`) ⑥ 전사 표준 승급(4레이어·네이밍). (개발환경 정리·화면 녹화 항목은 사용자 요청으로 제외)
