...
| **CLK(P1.7)/MOSI(P2.15) 물리 핀 무신호** — CS 어서트 후 CLK 토글 0회, MOSI 무변화. "zero-clock CS 프레임". MCSPI 드라이버가 클럭을 구동하지 않음. (D1 CLK 프로브 연결은 R35(cont.)에서 proven good) | R35 |
...
| **Saleae 계측 체인 proven** — spi0 known-good(12.5 kHz, 0xFF×128B)로 D1(CLK) 프로브 연결·BP P1.7=SPI0_CLK 물리 매핑·계측 정확도 전부 확인. 동일 프로브·핀에서 spi0=1024클럭·cc3351=0클럭 → 측정·배선·pinmux·물리 매핑 원인 배제. | R35(cont.) |
| **cc3351 CLK 침묵 원인 = 펌웨어/SoC SPI 동작 자체** — 배제 완료: 측정오차·프로브 불량·배선 오류·pinmux·물리 매핑. 잔여 후보: MCSPI_transfer 미호출·채널 enable·기능 클럭. | R35(cont.) |
...
**R35 현황**: CS 어서트 물리 확인. CLK/MOSI 무신호 — MCSPI_transfer 미호출 또는 호출 후 CLK 미출력. `evaluations/spi0` loopback vs 물리핀 실증 미확인. R36으로 이월.
...
- BoosterPack 헤더 ↔ SoC 핀 물리 대응표 — CLK 경로(P1.7=SPI0_CLK/A11/J1.7) R35(cont.) 실측 확인. MOSI/MISO/CS는 spi0 known-good 디코드 정합으로 간접 확인 수준.
