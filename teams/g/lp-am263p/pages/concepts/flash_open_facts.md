...
| **CLK(P1.7)/MOSI(P2.15) 물리 핀 무신호** — CS 어서트 후 CLK 토글 0회, MOSI 무변화. "zero-clock CS 프레임". MCSPI 드라이버가 클럭을 구동하지 않음. ⚠️ D1(CLK) 프로브 연결 미확정(CPOL=0, idle=LOW) — D0 포착 정상이라 전압 OK이나 D1 닿음 자체 재확인 필요. | R35 |
