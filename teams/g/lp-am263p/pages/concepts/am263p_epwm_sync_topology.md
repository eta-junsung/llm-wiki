...
source: AM263P TRM SPRUJ55D §7.5.6.4.3.3 Time-Base Counter Synchronization (printed pp.651–654, raw [[am263p_trm]] ch07_5_controlss.md :5683–5953) — PDF figures 7-181/7-182 직독 (2026-06-11) + 8kw EPWM0 fan-out 실측 검증 (commit 4014901, 2026-06-11)
date: 2026-06-11
---
...
EPWM SYNC는 **모듈별 독립 MUX(fan-out)** 다 — 데이지체인(직렬 전파)이 **아니다**. 각 EPWMx가 자기 `EPWMSYNCINSEL`로 **공용 SYNCOUT 풀에서 소스 1개**를 고른다(Table 7-154). source→target **한 hop의 내부 지연은 고정값**(`TBCLK==EPWMCLK`면 **2×EPWMCLK**, `TBCLK<EPWMCLK`면 **1×TBCLK**)이고 **target 인덱스·체인 위치와 무관·누적 없음**. ⟹ **한 소스를 여러 target이 동시에 선택하면 모두 같은 지연** → 상호 정수클록 스큐 0. 8kw의 ~11ns 비대칭은 "master 0-hop vs slave 1-hop"의 **hop 수 차이**(딱 2×EPWMCLK≈10ns @200MHz)이며, **더미 EPWM0 fan-out**으로 EPWM2/4/7 모두 1-hop化 → 비대칭 ~22 ns→**±2 ns** (**검증됨**, `4014901`, 2026-06-11).
...
**문서상 예측 = 같은 지연(상대 스큐 0, 정수클록 단위) — ✅ 실측 검증 완료 (`4014901`, 2026-06-11).** 근거(아래 §지연 모델):
- TRM 지연 공식은 "**control module → target module**" **한 쌍당** 적용되는 고정값이고, **target 인덱스가 식에 없다**. EPWM4·EPWM7 둘 다 EPWM0로부터 정확히 1 hop → 각각 2×EPWMCLK(=`TBCLK==EPWMCLK`일 때) → **상호 정렬**.
- SYNC 분배가 **per-module MUX(병렬 fan-out)** 이므로(아래 §토폴로지) "EPWM0 다음에 EPWM4, 그다음 EPWM7" 같은 **순차 전파가 존재하지 않는다** — 둘은 같은 net을 동시에 관측한다.

**실측 결과 (8kw `4014901`)**: 더미 EPWM0(output-less) fan-out으로 EPWM2/4/7 모두 1-hop화. 비대칭 **~22 ns → ±2 ns**. 정수클록분(10 ns) 제거 확인. 잔여 ±2 ns = 5 ns 양자화 ± sub-clock(라우팅/측정 잔류) — TRM 모델 해상도 밖. "정수클록 스큐 0"은 확인됨. Saleae 500MS/s(2 ns 격자) 4ch, 100/150/250/400 ns 4-DT sweep 전 항목 PASS.
...
|---|---|---|---|---|
| ~~구 (EPWM4 master→EPWM7)~~ | 0 | 1 | ~10ns(+레그간 0 vs 1-hop) | 비대칭. 8kw P1~d01fc0a 사용됨 |
| **fan-out: 더미 EPWM0→EPWM2/4/7 전부** | 1 | 1 | **±2 ns** | ✅ **8kw `4014901` 검증** — 정수클록 스큐 0 확인, 잔여는 양자화+sub-clock |
| 외부 SYNC(EXTSYNCIN)→둘 다 | 1 | 1 | **0**(정수클록) | 동일 원리, 미검증 |
