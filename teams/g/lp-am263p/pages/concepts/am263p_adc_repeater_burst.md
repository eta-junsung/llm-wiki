...
source: AM263P MCU+ SDK v2 `source/drivers/adc/v2/adc.h`·`adc.c` 함수 시그니처 실측 + 8kw-ev-wpt-tx `feature/adc-repeater-burst` PR#11 squash 머지(main `2c4ff85`, CI pass) 적용 코드(`src/bsp/eta_bsp_adc.c`) 역산·실보드 검증. TRM ch07_5_controlss.md(7.5.2.6.2 Trigger Repeaters) (2026-06-29). §3 N=16 버스트 라이브 타이밍 인입 — Saleae 트리거당 OSINT 1회(85 kHz)·버스트 3.12µs·cadence ≤195ns([[adc_repeater_burst_timing]]) (2026-06-30)
date: 2026-06-30
---
...
- **검증 ✓**: 스코프로 OSINT 85 kHz 확인 + 6채널 라이브 갱신(0/3.3V 인가) — PR#11 squash 머지, main commit `2c4ff85`, CI build pass. **N=16은 현 운영 값**. 최종 N 튜닝(노이즈 FFT 실측)은 후속 과제(아래 빈자리). 레지스터 readback(PPB COUNT/NSEL/SHIFT)은 미수행 — 검증법은 [[am263p_adc_ppb_averaging]] §제안 검증법 참조.
- **실측 타이밍 (2026-06-30, Saleae 디지털 — [[adc_repeater_burst_timing]])**: ADC0 단독 마킹(GPIO95=OSINT ISR).
  - **트리거당 OSINT 정확히 1회** = 85.03 kHz (12,260/12,260, 누락·중복 0) → 리피터가 한 트리거에 16변환을 몰아넣음 실증(분산 누적이면 85k/16≈5.3 kHz여야 함).
  - 트리거(EPWM0 ZERO)→버스트끝(ISR 진입) = **3.12 µs** = 주기(11.76 µs)의 **26.5%**; ISR 실행 0.304 µs. 실효 cadence **≤195 ns/변환**(상한 — IRQ 진입지연 포함, 정적 285 ns보다 빠름; [[am263p_adc_instance_allocation]] §변환시간 예산).
  - ADC1(2 SOC=32변환) 추론 ~6.2 µs ≈ 53% → 오버런 없음 **[추론, 미측정]**.
...
- **사실 (8kw 적용·검증)**: N=16 리피터 버스트 전환 구현·스코프 OSINT 85 kHz·6채널 라이브 검증(PR#11 squash 머지, main `2c4ff85`, CI build pass). **2026-06-30 Saleae 정밀 타이밍 인입**: ADC0 트리거당 OSINT 1회(85.03 kHz)·버스트 3.12 µs(26.5%)·cadence ≤195 ns([[adc_repeater_burst_timing]]). 레지스터 필드값 readback(PPB COUNT/NSEL/SHIFT)은 미수행 — 검증법은 [[am263p_adc_ppb_averaging]] §제안 검증법 참조.
- **가설**: 직교성 표의 그룹지연·억제 특성은 신호처리 일반론 — 8kw 실신호 노이즈 스펙트럼으로 검증 안 됨.
- **빈자리 (봉합 말 것)**:
  - **최종 N 튜닝 미완** — N=16은 현 운영 값(PR#11 머지). 스코프 FFT로 노이즈 거동(백색 vs 스위칭 상관) 측정 후 √N 평균 유효성·최적 N 판단 필요.
  - **변환시간 예산 상한 정밀도** — N 상한 ~41은 정적 산정([[am263p_adc_instance_allocation]]), tLAT 마감 보정·데이터시트 최소 S+H 미반영. 라이브 실측은 **1차 인입됨**(N=16 버스트 ≤3.12 µs, cadence ≤195 ns < 정적 285 ns) 단 **IRQ 진입지연 분해 미수행**이라 순수 cadence·~41 정밀 상한은 미확정 유지.
...
- [[adc_repeater_burst_timing]] — N=16 버스트 트리거→버스트끝 3.12 µs·OSINT 85 kHz 라이브 실측(§3 환원처).
