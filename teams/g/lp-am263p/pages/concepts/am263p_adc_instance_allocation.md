---
tags: [concept, am263p, adc, sysconfig, design-rule, platform]
source: 사용자 제공 설계 규칙 (2026-06-08) — 8kw-ev-wpt-tx(adc 브랜치) J3.28/J3.27 ADC 핀 추가 + 실보드 검증에서 도출. 6채널 완성(commit c512e3b) 반영 (2026-06-09). 변환시간 예산 절 추가 — TRM ch07_5_controlss.md + SDK 클럭 인용 (2026-06-26). 리피터 예산 비대칭 분배 보강 + 차기 채널 배치 권고 추가 (2026-06-29, ADC 필터 전환 검토 세션). 변환시간 예산 절을 TRM Table 7-123 정밀값으로 갱신·정정 — tSH=80ns(ACQPS=15)·tEOC=205ns·tLAT=220ns, cadence≈285ns, N 상한 ~41 (2026-06-29, 리피터 N 상한 정밀 산정 세션). 첫 라이브 실측 인입 + stale 정정(8kw 현 운용 = 리피터 버스트 N=16, PPB 누적/N=64 아님) — Saleae 실측 N=16 버스트 트리거→버스트끝 3.12µs(26.5%)·cadence ≤195ns([[adc_repeater_burst_timing]]) (2026-06-30)
date: 2026-06-30
---

# AM263P ADC 인스턴스 vs SOC(채널) 배치 결정 가이드라인

> **AM263P 플랫폼 설계 규칙 정본.** 신호를 어느 ADC 인스턴스/채널(SOC)에 배정할지 결정하는 기준. 형제 정본: [[am263p_adc_rti_trigger]](트리거 결선·EOC ISR), [[am263p_syscfg_soft_vs_hard_assign]](논리↔물리 배정 함정). 8kw 적용: [[adc_pinmap]].

## 전제 교정 (중요)

**여러 ADC 인스턴스를 쓰는 게 덜 안정적인 게 아니다.** AM263P 같은 모터·전력제어용 MCU가 ADC0~ADC4를 둔 이유 자체가 "동시에 여러 신호를 잡으라"는 것 — 멀티 인스턴스는 정상·표준 사용법이다. 진짜 차이는 **안정성이 아니라 ① 타이밍(동시성) ② 펌웨어 복잡도** 두 축이다.

## 핵심 하드웨어 사실 (아키텍처)

- **각 ADC 인스턴스(ADC0~4)는 독립 SAR 컨버터** — 자체 S/H·시퀀서·결과레지스터·ADCINT를 가진다. 인스턴스끼리는 완전히 독립적인 하드웨어.
- **한 인스턴스 안의 여러 SOC = 직렬 변환.** 인스턴스마다 변환기(SAR)·S/H가 **하나뿐**이라, 같은 인스턴스에 채널(SOC)을 여러 개 걸면 **트리거가 같아도 순차 변환**된다. → 채널 간 **샘플 스큐**(뒤 SOC는 앞 SOC 변환시간만큼 늦게 샘플) 발생, 그리고 **모든 SOC 변환시간 합이 트리거 주기 안에 들어와야** 한다. 인터럽트는 **마지막 SOC의 EOC**에 걸어 **한 ISR에서 인스턴스 결과를 coherent하게 read**한다.
- **서로 다른 인스턴스 = 병렬 변환.** 독립 하드웨어라 같은 트리거(예 RTI1)에 걸면 **물리적으로 동시 샘플**된다. 스큐 없음.

## 권장 결정 규칙 (안정성 아님 → 신호 특성 기준)

| 신호 성격 | 권장 배치 | 이유 |
|-----------|-----------|------|
| **동시성 중요 / 빠른 상관 신호** — 전류, V·I 쌍 등 제어루프에 들어가는 것 | **서로 다른 인스턴스 + 공통 트리거** | 동시 샘플 → 위상·전력 계산 정확. 멀티 ADC의 존재 이유 |
| **느린 / 무상관 신호** — 온도, 느린 전압 | **한 인스턴스에 SOC로 몰기, 마지막 EOC에 인터럽트 1개** | 스큐 무의미, 펌웨어 단순(ISR/int_xbar/ready-flag 절약) |

## 펌웨어 복잡도 축

인스턴스를 하나 늘릴 때마다 **+ISR 1개, +int_xbar output 1개, +ready 플래그 1개**가 든다(8kw ADC0 추가가 정확히 이 비용). 그래서 **"동시성이 필요 없는 신호는 한 인스턴스에 먼저 채우는 것"**이 펌웨어를 단순하게 유지하는 타당한 **tiebreaker**다 — 단 **복잡도 절감이지 안정성은 아니다.**

## 유일한 실제 불안정 (overflow)

한 인스턴스에 SOC를 **과적재**해 **변환시간 합 > 트리거 주기**가 되면 → 다음 트리거 전에 시퀀스를 못 끝내 결과가 **밀리거나 덮어씌워진다(overflow).** 이게 유일한 실제 instability이고, **멀티 인스턴스로 분산하면 오히려 해소**된다.

## ADC 변환시간 예산 & 리피터 N 상한 (정밀 산정 — 정적, 2026-06-29 갱신)

> 종전 2026-06-26 정적 산정(단일변환 ~315 ns·~37 변환/주기)을 TRM **Table 7-123** 정밀값으로 **갱신·정정**. 종전 수치는 **틀린 게 아니라 보수(과대)** — 정밀 cadence는 더 작아 N 상한이 ~41로 늘어난다. (라이브 레지스터 실측은 여전히 미수행 — 아래 빈자리.)

**리피터 N 상한을 결정하는 모델**

리피터(트리거 리피터 = NSEL+1 back-to-back 버스트, TRM `ch07_5_controlss.md`:751–789·:789)의 N 상한은 다음 부등식에서 나온다:

> **Σ(인스턴스 내 모든 SOC의 리피트 횟수) × (1회 직렬 cadence) ≤ 트리거 주기**

세 항의 결정 인자:
- ① **트리거 주기** = ADC 샘플레이트(8kw EPWM0_SOCA 85.032 kHz → **11,760 ns**).
- ② **1회 직렬 cadence** = tSH + tEOC. 이를 다시 결정하는 것: **SYSCLK**(tSH·tEOC 모두 SYSCLK 도메인) · **ADCCLK prescaler(÷4)** · **ACQPS/S+H 윈도우** · **분해능(12bit 고정)** · **tLAT 마감 보정**(직렬 N개 중 마지막은 결과 래치까지).
- ③ **인스턴스 내 SOC 개수** — SAR 엔진 1개를 공유하므로, 한 채널(SOC)의 N은 같은 인스턴스의 다른 SOC들과 **합 예산을 나눠** 쓴다.

**클럭 체인 (읽어 확인):**
- **SYSCLK = 200 MHz** (1 cyc = 5 ns) = CONTROLSS_PLL/2. **ADC base clock = SYSCLK 직결** (TRM :303 "The base ADC clock is provided directly by the system clock (SYSCLK)").
- **prescaler ÷4** — `ADC_CLK_DIV_4_0`(8kw `example.syscfg`:67); 이 enum 값 = **6** = `ADCCTL2.PRESCALE` 필드값(SDK `source/drivers/adc/v2/adc.h`:185 "ADCCLK = (input clock) / 4.0") → **ADCCLK = 50 MHz** (1 cyc = 20 ns). ADCCLK은 변환 phase에만 active, S+H 윈도우 동안 gated off (TRM :303).

**1회 직렬 cadence (SYSCLK 도메인 정밀값 — Table 7-123 PRESCALE=6 행, :2171):**
- **tSH = (ACQPS+1) × SYSCLK = 16 × 5 ns = 80 ns.** ACQPS 레지스터값 = **15**. 근거: 생성 코드 `ADC_setupSOC(.., 16)`의 마지막 인자 sampleWindow=16(`Release/syscfg/ti_drivers_open_close.c`:134 등 전 SOC), SDK가 레지스터에 `sampleWindow − 1`을 기록(`adc/v2/adc.h`:920; 의미 :888 "acquisition window duration in SYSCLK cycles") → ACQPS=15. tSH 정의: Table 7-122 :2116 "(ACQPS + 1) SYSCLK cycles".
- **tEOC = 41 SYSCLK = 205 ns** — S+H 끝 ~ 다음 변환 S+H 시작 가능 시점 (Table 7-123 :2171 PRESCALE=6 행; 정의 Table 7-122 :2118).
- **tLAT = 44 SYSCLK = 220 ns** — S+H 끝 ~ 결과 ADCRESULTx 래치 (:2171 / 정의 :2117).
- ⇒ **1회 직렬 cadence ≈ tSH + tEOC = 80 + 205 = 285 ns** (다음 변환이 시작 가능한 간격). 직렬 N개 중 마지막은 tLAT(220 ns)까지여서 +15 ns 마감 보정 — **미적용**(아래 빈자리).

**주기당 변환 / N 상한:**
- 주기당 최대 변환 ≈ 11,760 / 285 ≈ **41**.
- **단일채널 인스턴스(ADC0/2/3/4)**: 한 SOC의 리피트 ~**41**까지.
- **ADC1(SOC0+SOC1, 2 SOC)**: 두 SOC의 리피트 **합 ≤ ~41** — 균등 분배 강제 아님, **비대칭 가능**(예: SOC0 N=30 + SOC1 N=10, 합 40×285 ≈ 11.4 µs < 11.76 µs).
- 리피터 모듈은 인스턴스당 **2개**·각 NSEL 독립(TRM :753). "2채널이면 상한 ÷2"는 두 채널에 **균등한** 깊은 버스트일 때만 성립 — **합 예산 안의 비대칭 분배**가 정확한 모델.

**현 배치 overflow 마진 (리피터 버스트 N=16 — 2026-06-30 첫 라이브 실측):**
- ★ **현 운용 = 리피터 버스트 N=16**(`ETA_ADC_OVERSAMPLE_LOG2=4`, PR#11 main `2c4ff85`, 코드 트리 `6993a40` 재확인). 종전 이 절의 "리피터 미사용·PPB 누적 모드·8kw N=64(LOG2=6)" 서술은 **PR#11로 stale → 정정**. 한 트리거당 16변환을 백투백으로 몰아넣는다(분산 누적 아님). 정본 [[am263p_adc_repeater_burst]].
- **첫 라이브 실측** ([[adc_repeater_burst_timing]], Saleae 디지털 캡처): ADC0 트리거(EPWM0 ZERO)→버스트끝(OSINT ISR 진입) = **3.12 µs** = 주기(11.76 µs)의 **26.5%**, ISR 실행 0.304 µs 별도 → 마진 충분. **트리거당 OSINT 정확히 1회**(85.03 kHz, 12,260/12,260)로 "한 트리거 N회 버스트"가 실증됨(분산 누적이면 85k/16≈5.3 kHz여야 함).
- **정적 285 ns vs 실측 ≤195 ns** — 측정 cadence(상한)가 정적 보수치보다 빠름:

  | cadence 모델 | 1변환 | N=16 버스트 | 출처 |
  |---|---|---|---|
  | 정적 (보수 상한) | 285 ns (tSH 80 + tEOC 205) | 4.56 µs | TRM Table 7-123 |
  | **실측 (상한)** | **≤195 ns** | **3.12 µs** (IRQ 진입지연 포함) | [[adc_repeater_burst_timing]] |

  실측 ≤195 ns는 3.12 µs에 OSINT→IRQ 진입지연이 섞인 **상한**이라 순수 cadence는 더 작다(195 ns를 순수 cadence로 단정 금지). 정적 285 ns는 보수 상한으로 여전히 유효.
- ADC1(2 SOC=32변환) 추론 ~6.2 µs ≈ 주기의 53% → 오버런 없음 **[추론 — 본 측정은 ADC0 단독, ADC1 미측정]**.
- (개념 — PPB *누적* 모드와의 대비) PPB 누적 모드는 한 트리거당 N회가 아니라 N개 트리거에 걸쳐 누적되어 변환시간 예산이 **N과 무관**하다([[am263p_adc_ppb_averaging]] §5). 8kw는 종전 이 모드(N=64)를 쓰다 현재 리피터 버스트(N=16)로 전환 — 리피터는 위 표대로 **N×cadence가 예산에 묶인다**.

**리피터 N=64 기각 ≠ 리피터 기각 (정본 명시):**
- 리피터 버스트는 N회를 **한 트리거 안에** 몰아넣는다: N=64 → 64×285 ≈ **18.2 µs > 11.76 µs** → 예산 초과 → **N=64 기각** (근거 [[am263p_adc_rti_trigger]] §5·[[adc]] A3.5).
- 그러나 **N ≤ ~41은 예산 내** → **저-N 리피터는 살아있는 선택지** — 기각된 건 N=64이지 리피터 자체가 아니다. **현 채택 = 리피터 버스트 N=16**(2026-06-30 실측 26.5% 점유, 마진 충분; [[adc_repeater_burst_timing]]).

**종전 wiki 정정 (코드/TRM 표가 정본):**
- ~~"ACQPS = 16, window = 85 ns"~~ → **오인.** SysConfig sampleWindow=16이고 SDK가 −1을 기록 → ACQPS 레지스터=**15**, tSH = (15+1)×5 = **80 ns**.
- ~~"Conversion ≈ 11.5 ADCCLK → 230 ns"~~ → Table 7-123 정밀값 **tEOC = 41 SYSCLK = 205 ns**가 정본. (TRM :305의 "≈11.5 ADCCLK"는 approximate 서술; 정밀표가 우선.)
- ~~"단일 변환 ≈ 315 ns → ~37 변환/주기"~~ → 과대(보수) 변환시간 기반. **틀린 게 아니라 보수치**; 정밀 cadence ≈ 285 ns → ~**41**/주기. 둘 다 정적 산정.
- (정정 아님·일치 확인) "ADCCLK 50 MHz" = 200÷4 — 합치.

**남은 빈자리 (봉합 말고 호명):**
- **변환 phase 사이클 정의 TRM 내부 불일치**: :305 "≈11.5 ADCCLK" vs :2088 "≈10.5 ADCCLK" (둘 다 approximate). 정밀표 Table 7-123(SYSCLK 절대값)을 정본으로 우회했으나, **ADCCLK 환산 정의는 미확정**.
- **마지막 변환 마감 공식 미확정**: 직렬 N개 총시간을 (N−1)(tSH+tEOC) + (tSH+tLAT)로 봐야 하는지 — 마지막 cadence를 tEOC(205)→tLAT(220)로 보면 **N 상한을 1~2 깎을 수 있음**. 2026-06-30 실측(N=16 버스트가 정적식 4.56 µs보다 빠른 ≤3.12 µs)이 "정적식이 보수"라는 데이터 포인트는 주나, IRQ 진입지연 분해 미수행이라 **공식 확정엔 불충분** — 미해결 유지.
- **데이터시트(SPRSxxx) 최소 S+H 윈도우(ns) 미인입** — 현 tSH=80 ns가 12bit 최소 요구를 만족하는지, 리피터 burst에서 tSH 단축 여지 있는지 **미확인**. (TRM :890/:2088이 "데이터시트가 최소 윈도우를 규정"으로 위임.)
- **라이브 실측 — 1차 인입됨 (2026-06-30), 단 부분적**: N=16 리피터 버스트의 **트리거→버스트끝 = 3.12 µs**(IRQ 포함)·**실효 cadence ≤195 ns**가 스코프로 측정됨([[adc_repeater_burst_timing]]). 정적 285 ns/주기당 ~41 모델은 **보수 상한으로 살아있고** 실측이 더 빠름. 단 **IRQ 진입지연 분해·레지스터 readback·ADC1 ISR 횟수는 여전히 미수행** — 순수 cadence 절대값·~41 정밀 상한은 미확정 유지.

## 8kw 현황 (예시)

현재 [[adc_pinmap]] 배치 — **6채널 완성, 물리 인스턴스 5개**(commit c512e3b 기준):

- **ADC1 = Temp_Module2(SOC0) + GA_Iin_SEN(SOC1)** — 동일 인스턴스 2 SOC, **1 ISR(IRQ146)**. 직렬 변환·스큐 발생.
- **ADC0 = I_COIL_SEN**, **ADC4 = I_LCC_SEN** — 각각 별도 인스턴스(전류 = 상관 신호), SOC0 단독.
- **ADC2 = Temp_Module1**, **ADC3 = GA_Vin** — 각각 별도 인스턴스, SOC0 단독(신규 2채널).
- ⚠️ ADC1이 입력 전류(GA_Iin_SEN, 상관 신호)를 온도(저속 TEMP_MODULE2)와 동거시켜 직렬 변환됨 — 전류 채널이 온도 변환시간만큼 스큐되고 리피터 깊이도 공유 예산으로 제약받는다. **현 보드는 PCB 라우팅 고정이라 그대로 운용**.
- **다음 PCB 설계 시 최적 배치 권고 (2026-06-29)**:
  - 온도(TEMP_MODULE1 + TEMP_MODULE2) → **한 인스턴스에 묶기** (저속·무상관, ISR 1개로 절약).
  - 빠른 신호 I_COIL_SEN·GA_Iin_SEN·GA_Vin → **각자 별도 인스턴스** (동시샘플, 스큐 없음, 리피터 깊이 독립).
  - 펌웨어-only 변경으로 실현 가능한지는 **불확실** — 물리 AIN 핀 라우팅 의존(소프트 재배치 한계 [[am263p_syscfg_soft_vs_hard_assign]]). **다음 PCB 설계 단계에서 핀 배정 반영** 필요.

## 한 줄 요약 (다음 보드 설계 시)

상관·고속 신호는 인스턴스를 나눠 **병렬·동시샘플**, 무상관·저속 신호는 한 인스턴스에 모아 **ISR 절약**. "인스턴스 채우는 순서"는 안정성이 아니라 **동시성 요구 + 인스턴스당 변환시간 예산**으로 결정. 8kw 차기 권고: TEMP×2 한 인스턴스, I_COIL/GA_Iin/GA_Vin 별도 인스턴스. ★ 실보드에선 **PCB 라우팅이 신호→인스턴스/채널을 거의 고정**하므로, **설계 단계에서** 위 기준으로 핀을 배정해두는 게 핵심.

## 사실 / 가설 / 모름 가름

- **사실 (아키텍처)**: SAR 직렬(한 인스턴스 다중 SOC)/병렬(인스턴스 간)·인스턴스 독립(자체 S/H·시퀀서·결과레지스터·ADCINT). 인스턴스 추가 = +ISR/+int_xbar/+ready-flag.
- **사실 (변환시간 — 2026-06-29 정밀 갱신)**: 1회 직렬 cadence ≈ **285 ns**(tSH 80 ns + tEOC 205 ns), tLAT 220 ns, ADCCLK 50 MHz, tSH는 SYSCLK 도메인·ACQPS=15. 주기당 ~**41 변환**, 리피터 N 상한 ~41(단일채널)/합≤41(ADC1). ~~종전 단일변환 315 ns·~37 변환~~ → 보수치였고 Table 7-123 정밀값으로 정정(위 "ADC 변환시간 예산 & 리피터 N 상한" 절: TRM :303/:2088/:2116–2118/:2171 + `adc/v2/adc.h`:185·:920 + `ti_drivers_open_close.c`:134). → **2026-06-30 라이브 실측 1차 인입**: N=16 버스트 cadence ≤195 ns(상한) < 정적 285 ns([[adc_repeater_burst_timing]]). 단 **IRQ 지연 분해·레지스터 readback은 여전히 미수행**.
- **△ 미검증(실측)**: **다중 인스턴스 동시 트리거의 정밀 동시성 미실측** — "동시 샘플"은 아키텍처 근거이며 스큐 상한을 스코프로 잰 적은 없음. (트리거는 RTI1 → EPWM0_SOCA로 전환됨, [[am263p_adc_rti_trigger]] §5.)
- 레지스터 수치 출처가 필요하면 [[am263p_trm]] ADC 챕터 demand-ingest.

## 관련 페이지

- [[am263p_adc_rti_trigger]] — RTI 공통 트리거 결선·EOC ISR-flag 패턴(이 규칙의 "공통 트리거" 구현부). §3 다핀 확장의 "어떻게 정하는가"를 이 페이지가 보강.
- [[am263p_syscfg_soft_vs_hard_assign]] — 논리 인스턴스명↔물리 페리페럴 배정 함정(soft 재배치). 인스턴스를 정한 뒤 **물리로 고정**하는 방법.
- [[adc_pinmap]] — 8kw 실제 신호→인스턴스/채널 배치.
- [[am263p_adc_ppb_averaging]] — PPB HW 오버샘플 평균. N은 트리거에 걸쳐 누적되므로 변환시간 예산과 무관(repeater와 대비).
- [[am263p_adc_repeater_burst]] — 현 8kw 채택 모드(리피터 버스트 N=16). 버스트 N은 cadence×N ≤ 트리거 주기에 묶임.
- [[adc_repeater_burst_timing]] — N=16 버스트 트리거→버스트끝 3.12 µs·cadence ≤195 ns 라이브 실측(이 절 정적 285 ns 모델의 첫 실측 병기).
- [[am263p_trm]] — ADC 챕터(SOC 변환시간 = 트리거 주기 예산 계산용).
