...
OSPI 부팅 시 SBL이 chip을 8D로 두는지, 1S로 두는지 **미확인**. R20의 DQ1-only는 "SBL이 애초에 8D로 안 둔다"(가지 α)를 시사할 수 있다. SBL 소스 또는 TRM OSPI boot 절차에서 확인 필요. → [[flash_open_facts]] 모름/미확인.
> ※ 부팅 스트랩 정정: 이 보드는 OSPI 4S(`1,1,1,1`)가 아니라 **xSPI 8D SFDP(`0,0,1,1`)** 로 부팅한다(octal-only 칩 — [[ospi_boot_mode_strap]]). 위 "4S Quad" 전제로 적힌 옛 서술은 그 관점에서 읽을 것.
...
> **층위 구분**: 이 페이지의 `skipHwInit`/`flashFixUpOspiBoot` 비대칭은 **cc3351 app 런타임 `Flash_open()`** 문제다. 8kw **standalone 부팅 무부팅**은 별개 층위(부트모드 스트랩 미스매치 — [[ospi_boot_mode_strap]], 2026-06-12 해소)이니 섞지 말 것. ※ §게이트 표의 `QE bit set (1S_1S_4S, 4S_*)`는 SDK 드라이버의 **범용** 분기로, octal-only IS25LX256에는 해당 4S/quad 경로 자체가 비활성(quad/QE 부재 — [[ospi_boot_mode_strap]] §2).
