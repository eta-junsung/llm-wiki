...
**A5·A6 번호에 속하지 않는 별도 트랙** — Io(I_COIL_SEN) ADC 85 kHz 스위칭 노이즈 제거용 재귀형 IIR biquad LP. **구현 완료**(branch `feature/adc-noise-fft`, commit `d4542c9`, 2026-07-01): `eta_alg_iir_lpf`(ALG) + App `−Vadc/2` offset 파라미터화, 빌드 경고 0. **A6(SW 이동평균)은 코드에 없어 대체 대상 없음** — A5 리피터 버스트(N=16) 위에 새로 얹는 구조로 확정. **실보드 파형·노이즈 검증은 미실행** — A5 노이즈 측정(FFT) 세션에 편승 예정, 이때 `−Vadc/2` offset 최종값도 함께 확정. 정본 [[io_iir_lpf]], 현황 [[status]].
