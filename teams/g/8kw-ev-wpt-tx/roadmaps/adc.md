...
**남은 절차**:
1. 스코프 FFT로 노이즈 거동 측정 — **백색이면 √N 평균 유효(N↑)**, **스위칭 상관 피크면 트리거 위상이 더 큰 레버**(트리거가 EPWM에 위상고정 → 상관 노이즈는 coherent → √N 무효). 프로브 포인트(GA_Vin/GA_Iin DC핀)·결정 트리·MSOX3104T 설정은 정본 [[adc_noise_fft_probe]]. 이 측정으로 최종 N(또는 위상) 확정. ⚠️ **관련 주의(2026-07-01)**: 인접 FOD 트랙([[fod_i_coil_observation]] §3.5)에서 같은 스코프로 I_COIL_SEN CSV FFT를 2회 시도해 둘 다 물리적으로 불가능한 진폭이 재현돼 그 트랙은 보류·온타깃 raw 캡처로 전환했다(branch `feature/adc-raw-capture`, 스펙만 작성·구현 전, [[fod_i_coil_observation]] §4). 원인 미규명 — GA_Vin/GA_Iin 측정 전 진폭이 0~3.3V 범위 안인지 먼저 검산할 것. A5의 이 FFT 절차 자체도 온타깃 raw 캡처가 구현돼야 실행 가능하다(현재 리피터가 이미 블록평균해버려 raw를 볼 방법이 없음).
...
### io_iir_lpf — Io 2차 IIR 저역통과 필터 (A5/A6과 별개 트랙, 구현 완료·검증 대기)

**A5·A6 번호에 속하지 않는 별도 트랙** — Io(I_COIL_SEN) ADC 85 kHz 스위칭 노이즈 제거용 재귀형 IIR biquad LP. **구현 완료·main 병합**(PR #12, `953d4f3`, commit `d4542c9`, 2026-07-01): `eta_alg_iir_lpf`(ALG) + App `−Vadc/2` offset 파라미터화, 빌드 경고 0. **A6(SW 이동평균)은 코드에 없어 대체 대상 없음** — A5 리피터 버스트(N=16) 위에 새로 얹는 구조로 확정. **실보드 파형·노이즈 검증은 미실행** — A5 노이즈 측정(FFT) 세션에 편승 예정, 이때 `−Vadc/2` offset 최종값도 함께 확정. 정본 [[io_iir_lpf]], 현황 [[status]].
...
A2 완료(2026-06-09, c512e3b) · A3 완료(5/6채널, I_LCC_SEN 드롭, 2026-06-26) · A3.5 **완료**(트리거 EPWM0_SOCA + PPB HW 평균 N=64 HW 확정, main PR #6 `d673e74`) · **io_iir_lpf 구현 완료**(2026-07-01, `d4542c9`, A5/A6과 별개 트랙, 검증 대기). 다음(실행 순서): **A5 필터 안정화**(FOD 노이즈 측정 → 최종 N/트리거 위상, io_iir_lpf offset 확정 겸용) → **A6 SW 이동평균** → **A4 실보드 교차검증** — [[status]] 참조.
