...
| **firmware-layering** | BSP·HAL·ALG·App 4레이어 전면 재구성 (동작 불변) | — | ✓ **완료(2026-06-25, PR #5, `0830b5f`·`9cd0181`)**. ALG 레이어 의도적으로 비어있음(정상 — eta_alg_crc16·eta_alg_convert만 존재). 빈 경계(eta_alg_control·eta_hal_pwm)는 A3/P3 스펙 진입 시 생성. 정본 [[firmware_layering_8kw]] |
...
- **구조 트랙**: `firmware-layering` **✓ 완료(2026-06-25, PR #5)**. BSP·HAL·ALG·App 4레이어 전면 재구성, 동작 불변 실보드 검증. ALG 레이어는 의도적으로 거의 비어있음(eta_alg_crc16·eta_alg_convert만 존재 — 정상). 빈 경계(eta_alg_control·eta_hal_pwm)는 A3 스케일링 스펙·P3 보호 스펙 진입 시 생성. GUI 물리량 변환 펌웨어 회수 보류(A3 스펙 대기, 현재 gui.py PHYSICAL_COEFF 유지). 정본 [[firmware_layering_8kw]].
