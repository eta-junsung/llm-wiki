...
| **pwm** | EPWM 전력제어 (인버터 게이트 구동·보호·제어루프) | P0~P4 → [[pwm]] | **P1 ✓**(4핀 실보드 검증·shoot-through 0 `6e6b342`) **+ P2 △**(두 레그 `ETA_DEADTIME_NS` dead-time 단일소스 통일·150/300ns 스윕 4ch 실측 `8046744`). **주파수 85kHz·dead-time 100~400ns 확정.** 다음 = P3 보호 / 보호신호 스펙 |
...
- **활성 트랙**: `pwm` 작업 호([[pwm]], P0~P4) — **P1 완료**(EPWM2/4/7 4핀 실보드 검증, 레그2 두 모듈 SYNC 상보, shoot-through 0) **+ P2 dead-time 단일소스 통일**(두 레그 `ETA_DEADTIME_NS` 하나·150/300ns 스윕 4ch 실측, `8046744`). **주파수 85kHz 고정·dead-time 100~400ns 조정 확정(2026-06-10)**, 레그2 SYNC dead-time은 plat 정본 [[am263p_epwm_module_sync_deadtime]] 승격. 다음 = P3 보호(trip-zone) / 보호신호·게이트 극성 회로도 스펙 확보.
