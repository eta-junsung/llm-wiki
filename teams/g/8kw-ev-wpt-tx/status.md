...
## 다음 작업: ADC A5 필터 안정화 → A6 SW 이동평균 → A4 실보드 교차검증 → PWM P4 제어루프

**다음 시작점**: **I_COIL_SEN 노이즈 측정 → ADC 안정화 → FOD 관찰** (branch **`feature/adc-noise-fft`**, 절차 정본 [[fod_i_coil_observation]]). **안정화-선행 3단계 순서** (FOD 문서의 "정상·이물 동시캡처 후 필요시 안정화" 흐름을 σ 선(先)감소로 재배열):
