...
## 다음 작업: Io IIR LPF 구현(구현만) → ADC A5 필터 안정화 → A6 SW 이동평균 → A4 실보드 교차검증 → PWM P4 제어루프

**다음 시작점**: **Io 2차 IIR LPF(io_iir_lpf) 구현** — 구현만(빌드 통과 + on/off 가능), 스펙 정본 [[io_iir_lpf]]. **그다음** ADC 안정화 트랙으로 진입(아래). ★필터 실보드 검증은 ADC 안정화 이후.

**ADC 안정화 (필터 구현 후)**: **I_COIL_SEN 노이즈 측정 → ADC 안정화 → FOD 관찰** (branch **`feature/adc-noise-fft`**, 절차 정본 [[fod_i_coil_observation]]). **안정화-선행 3단계 순서** (FOD 문서의 "정상·이물 동시캡처 후 필요시 안정화" 흐름을 σ 선(先)감소로 재배열):
> 1. **정상에서 노이즈 측정**: 전력단 스위칭 중, I_COIL_SEN(J3.28, ADC0)에 스코프(DC 레벨 baseline / AC+FFT / 시간축 CH3 HS1 J4.39 ↔ CH4 GPIO95 J4.31) + GUI 10 Hz CSV(ch4 raw) → FFT로 **백색 vs 85 kHz 스위칭 상관** 판별 + σ(counts) 산출.
...
| **Io 2차 IIR LPF (io_iir_lpf)** | ✗ | 착수 예정(2026-07-01 미팅). Direct Form I 2차 biquad LP, 85kHz 코드계수(b0=b2=9.75e-6·b1=1.95e-5·a1=1.987473·a2=−0.987512). ALG biquad+App `−Vadc/2`. **구현만 먼저, 검증은 ADC 안정화 후.** 스펙 [[io_iir_lpf]] |
...
### 활성 트랙 — Io 2차 IIR 저역통과 필터 (io_iir_lpf) 구현 착수 (2026-07-01 미팅)

- **Io ADC 스위칭 노이즈 제거용 2차 IIR biquad LPF** 구현. 설계자 스펙 3종 수령(2026-07-01) → 정본 ingest [[io_iir_lpf]], 원본 `raw/io_iir_lpf/`.
