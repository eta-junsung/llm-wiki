...
**다음 시작점**: **PPB HW 블록평균(N=64) → SW 이동평균 전환** (A5+A6 통합 방향, 2026-06-29 결정). 두 작업:
1. `ETA_ADC_OVERSAMPLE_LOG2`(`src/bsp/eta_bsp_adc.h:28`) 낮춰 실질 85 kHz 달성 + SW ring buffer 이동평균 구현 (매 샘플 갱신, 스파이크 감지 + 노이즈 억제 동시).
2. **GUI에서 85 kHz 이동평균 데이터 측정 방법 모색** — 현재 UART5 텔레메트리 10 Hz로는 85 kHz 데이터 전달 불가. 버퍼 캡처·전송 레이트 상향 등 방법 검토 필요(미결).

A3.5(EPWM0_SOCA 트리거 + PPB HW 평균)는 **HW 검증으로 N=64 확정·완료**(main PR #6 `d673e74`). 정본 [[am263p_adc_rti_trigger]] §5·[[am263p_adc_ppb_averaging]]. (ADC 외 활성 트랙은 PWM P3 보호.)
...
| **ADC 실질 샘플링 85 kHz (A5, #7)** | ✗ | `ETA_ADC_OVERSAMPLE_LOG2`(`src/bsp/eta_bsp_adc.h:28`) 낮춰 85 kHz 달성, 노이즈 실측. A3.5(N=64 HW-first) 재검토. A4와 [추정] 독립 |
