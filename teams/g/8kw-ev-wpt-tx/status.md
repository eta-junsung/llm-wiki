...
스펙: **주파수 85 kHz 고정 확정**(2026-06-10), **dead-time만 가변(build-per-change, 100~400 ns 조정·실험 후 고정)**. UART5(EPWM15) 무충돌.
...
- 다음: **dead-time 100~400 ns 실험→최종값 고정**(주파수 85 kHz로 고정) → P3 보호(trip-zone) → **P4 제어루프 + ADC SOC 트리거 RTI→EPWM 전환** / 게이트 극성 회로도·보호신호 스펙 확보. 상세 [[pwm]].
...
- **PWM 잔여 스펙**: ~~스위칭 주파수~~ **85 kHz 고정 확정(2026-06-10)**. 보호(trip) 신호 소스 미정(P3 선결). dead-time 100~400 ns 실험→최종값 고정 예정.
