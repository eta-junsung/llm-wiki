...
## 다음 작업: ADC A3 물리량 변환 (3채널) → PWM P3 / ADC(A4) / UART5 Phase 2

**다음 시작점**: **A3 물리량 변환 — GA_Iin_SEN 다음 세션 진행. `tools/gui/gui.py` `PHYSICAL_COEFF[5]`(GA_Iin_SEN) 채널 상수 블록 작성 + 검증 데이터포인트 확인. I_COIL_SEN(`PHYSICAL_COEFF[4]`) ✓ 완료(commit 7335418, branch feature/adc-calculation).**
...
| 신호별 스케일링 (A3) | △ | **I_COIL_SEN ✓ 완료·검증**(7335418, 6.770 A/V, 검증 22.07 A). GA_Iin_SEN·I_LCC_SEN·GA_Vin·Temp×2 미교정. 변환은 GUI `PHYSICAL_COEFF` 단일 소스([[adc_scaling]]) |
...
- **A3 물리량 변환 진행 중**: ~~I_COIL_SEN~~ ✅ **완료·검증(2026-06-24, commit 7335418)** — SCALE≈6.770 A/V, OFFSET≈4.198 A, ADC_V=2.64V→22.07A(기대 22.1A) 일치. **아키텍처 확정: 변환은 GUI `PHYSICAL_COEFF` 단일 소스, MCU는 raw/mV만 전송(보류)**. 식·파라미터 [[adc_scaling]]. **잔여**: GA_Iin_SEN(다음 세션)·I_LCC_SEN·GA_Vin·Temp_Module1/2 미교정. [[adc_pinmap]] §미확인 항목 참조.
- **Vref 3.3V 확정 갱신**: `adc_pinmap.md`가 "VREFHI 1.8V(참고값)" 기술이었으나 `ETA_ADC_VREFHI_MV=3300` + 사용자 확인으로 **3.3V 확정**으로 갱신(2026-06-24).
