...
**다음 시작점**: ADC SOC 트리거 RTI1 → EPWM0(85 kHz) 전환 — EPWM0 fan-out 마스터(`4014901`)로 ADC SOC 트리거 연결 후, 고속 샘플 위에 PPB HW 평균 필터 순서. [[am263p_adc_rti_trigger]] §1(export 게이트 함정) 재점검·§4(PPB 오버샘플링) 착수 전 확인.
...
2. **ADC SOC 트리거 RTI1→EPWM0 전환 + PPB HW 평균 필터 (순위 2)**: 기존 순위 2(SW 필터)·순위 3(트리거 전환) 통합, 순서 뒤집음. ① **트리거 RTI1(1 kSPS) → EPWM0(85.032 kHz) 전환 선행** — EPWM0는 `4014901` 도입 output-less fan-out 마스터. 전환 시 트리거 export 게이트 함정([[am263p_adc_rti_trigger]] §1) 점검. ② **PPB HW 평균 필터 후행** — power-of-2 자동 평균(최대 1024샘플, CPU-free). SW 이동평균 대비 RAM 버퍼·루프 연산 불필요. 잔여 노이즈에만 SW 보완. 정본 [[am263p_adc_rti_trigger]] §4.
