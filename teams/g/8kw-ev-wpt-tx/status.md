...
**다음 시작점**: **리피터 버스트(N=16) 노이즈 FFT 실측 → 최종 N 확정** (branch `feature/adc-repeater-burst`). 스코프 FFT로 노이즈 거동 측정 — 백색이면 √N 평균 유효, 스위칭 상관 피크면 리피터 `repPhase`/`repSyncin` 위상동기가 더 큰 레버. 그 다음 **SW 이동평균 `eta_alg_filter`(ALG) 신설** — ISR write로 먼저 구현해 DMA 검증과 분리(권장순). 정본 [[am263p_adc_repeater_burst]]·로드맵 [[adc]] §A5·§A6.

**A5 진척 — PPB 누적(N=64) → 리피터 버스트 블록평균(N=16) 전환 (branch, 실보드 검증)**: 한 트리거 내 16회 백투백 변환 → PPB 블록평균 → **출력 85 kHz/인스턴스 유지**(누적 1.33 kHz와 대비). 스코프 OSINT 85 kHz + 6채널 라이브 검증(main 미머지). N=16은 placeholder(최종 N은 FFT 후). SDK API·`ADC_configureRepeater` 헤더 결함·HW/SW 필터 직교성 정본 [[am263p_adc_repeater_burst]]. EDMA 적재는 가능 확인(SDK)·채택 보류 [[am263p_edma_adc_offload]].
...
| **ADC 실질 샘플링 85 kHz (A5, #7)** | △ | **PPB 누적(N=64) → 리피터 버스트 블록평균(N=16) 전환**(branch `feature/adc-repeater-burst` `2af8642`), 출력 85 kHz/인스턴스. 스코프 OSINT 85 kHz·6채널 라이브 검증(main 미머지). `ETA_ADC_OVERSAMPLE_LOG2=4U`(`eta_bsp_adc.h:31`). 최종 N·노이즈 FFT 미완. 정본 [[am263p_adc_repeater_burst]] |
