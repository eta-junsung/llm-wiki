...
**다음 시작점**: **ADC A4 실보드 교차검증** 또는 **A5 (#7 — `ETA_ADC_OVERSAMPLE_LOG2`(`src/bsp/eta_bsp_adc.h:28`) 낮춰 실질 85 kHz 달성 + 노이즈 실측)**. A4·A5 선후 [추정] 독립(확인 필요). A3.5(EPWM0_SOCA 트리거 + PPB HW 평균)는 **HW 검증으로 N=64 확정·완료**(main PR #6 `d673e74`). 정본 [[am263p_adc_rti_trigger]] §5·[[am263p_adc_ppb_averaging]]. (ADC 외 활성 트랙은 PWM P3 보호.)
