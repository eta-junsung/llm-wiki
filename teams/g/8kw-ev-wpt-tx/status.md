...
**다음 시작점**: **FOD I_COIL_SEN 1차 관찰** (branch **`feature/adc-noise-fft`**) — 절차 정본 [[fod_i_coil_observation]]. 전력단 스위칭 중 상태에서 I_COIL_SEN(J3.28, ADC0)을 **트랙 A 스코프**(DC 커플링=이물 레벨 시프트 / AC+FFT=노이즈 백색 vs 85 kHz 상관 / 시간축 CH3 HS1 J4.39 ↔ CH4 GPIO95 J4.31) + **트랙 B GUI 10 Hz CSV**(ch4 raw)로 동시 캡처 → **정상 vs 이물** 비교로 검출성 `Δ/σ` 산정. ★ I_COIL_SEN(J3.28)은 CT→정류→RC평활된 **DC 포락선**이라 FOD 프로브로 적합(종전 "85 kHz 신호라 부적합" 정정). 측정 마커는 이미 코드에 있음(`ETA_BSP_ADC_DBG_MARK_IDX 0U`=I_COIL_SEN; GA_Vin 노이즈만 볼 땐 `3U`). ⚠️ OC 임계 이하 유지(같은 노드=I_COIL_OC 비교기 탭).
