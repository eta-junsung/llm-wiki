...
## 직전 완료 — Io 2차 IIR LPF(io_iir_lpf) 구현 (branch feature/adc-noise-fft, commit d4542c9, 2026-07-01)
...
## 직전 완료 — FOD I_COIL_SEN 1차 관찰: 트랙 A(외부 스코프) 보류 → 온타깃 raw 캡처 전환 (branch feature/adc-noise-fft, commit 620e62b, 2026-07-01)
...
## 다음 작업: ADC 온타깃 raw 캡처 구현 → A5 필터 안정화(FOD 노이즈 측정) → A6 SW 이동평균 → A4 실보드 교차검증 → PWM P4 제어루프

**다음 시작점**: **온타깃 raw ADC 캡처 구현** — I_COIL_SEN 버스트 샘플을 링버퍼에 캡처해 1회 UART 일괄 덤프하는 경로 설계·구현(캡처 트리거·버퍼 크기·덤프 패킷 포맷 전부 미정, [[fod_i_coil_observation]] §4). 외부 스코프(트랙 A)는 보류 상태이므로 **이 경로가 I_COIL_SEN 노이즈 실측의 유일한 남은 길**. 이 측정에서 io_iir_lpf의 `−Vadc/2` offset 최종값도 함께 확정.

**ADC 안정화**: **I_COIL_SEN 노이즈 측정 → ADC 안정화 → FOD 관찰** (branch **`feature/adc-noise-fft`**, 절차 정본 [[fod_i_coil_observation]]). **안정화-선행 3단계 순서** (FOD 문서의 "정상·이물 동시캡처 후 필요시 안정화" 흐름을 σ 선(先)감소로 재배열):
> 1. **정상에서 노이즈 측정**: ~~스코프(DC 레벨 baseline / AC+FFT)~~ — **트랙 A 보류**(위 직전 완료 참고, 진폭 이상 재현·원인 미규명). **온타깃 raw 캡처(§4, 미구현) + GUI 10 Hz CSV(ch4 raw)**로 대체 → FFT로 **백색 vs 85 kHz 스위칭 상관** 판별 + σ(counts) 산출.
...
> ⚠️ **잠복 충돌**: 디버그 마커 GPIO95(J4.31)를 `eta_hal_gpio`의 DBG_LOOP도 사용 → 후속 정리 필요(`6993a40` 커밋 메시지 명시). 마커는 측정용 디버그 스캐폴딩(측정 종료 후 제거 대상).
...
| **Io 2차 IIR LPF (io_iir_lpf)** | △ | **구현 완료**(branch `feature/adc-noise-fft`, commit `d4542c9`, 2026-07-01). `eta_alg_iir_lpf.{c,h}` biquad + `eta_adc_sample_t.filtered` + App 배선(배치 step 버그 수정·offset 파라미터화). 빌드 경고 0. **실보드 파형·노이즈 검증은 미실행**(ADC 안정화 트랙 대기). 스펙 [[io_iir_lpf]] |
...
### ✅ 완료 — Io 2차 IIR 저역통과 필터 (io_iir_lpf) 구현 (branch feature/adc-noise-fft, d4542c9, 2026-07-01)
...
### ⚠️ 진행 중 — FOD I_COIL_SEN 1차 관찰: 트랙 A(외부 스코프) 보류, 온타깃 raw 캡처 미구현 (branch feature/adc-noise-fft, 620e62b, 2026-07-01)

- I_COIL_SEN(J3.28) CSV FFT 2회 시도 모두 진폭 이상(pk-pk 20~36V, 0~3.3V 노드) 재현 → **원인 미규명**, 외부 스코프 트랙 보류. 상세 [[fod_i_coil_observation]] §3.5, 직전 완료 항목 참고.
- **다음 시작점 = 온타깃 raw ADC 캡처(버스트 캡처 + 1회 UART 덤프) 구현** — 캡처 트리거·버퍼 크기·덤프 패킷 포맷 전부 미정([[fod_i_coil_observation]] §4). 이게 없으면 I_COIL_SEN 노이즈 실측(A5 FFT·io_iir_lpf offset 확정 포함)이 막혀 있다.
