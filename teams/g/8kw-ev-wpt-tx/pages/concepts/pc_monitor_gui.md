...
source: 펌웨어 repo branch uart5 (commit ba241fa·979699d) + dead-time 컨트롤 추가 (commit 785b848, 2026-06-12) + 실보드 검증 2026-06-11
date: 2026-06-12
...
|-----|--------|-------------|--------------|
| ch0 | Temp_Module2 | ADC1 SOC0 | 미입수 (`—`) |
| ch1 | Temp_Module1 | ADC2 SOC0 | 미입수 (`—`) |
| ch2 | GA_Vin | ADC3 SOC0 | `— V` (분압비 미입수) |
| ch3 | I_LCC_SEN | ADC4 SOC0 | `— A` (감도 미입수) |
| ch4 | I_COIL_SEN | ADC0 SOC0 | `— A` |
| ch5 | GA_Iin_SEN | ADC1 SOC1 | `— A` |
...
```
REPO_ROOT  → gui.py 위치에서 3단계 위 (exe 실행 시 4단계)
TUNING_H   → REPO_ROOT/src/eta_bsp/eta_tuning.h
BUILD_DIR  → REPO_ROOT/build
FLASH_PS1  → REPO_ROOT/tools/jtag_flash/run_flash_node_8kw.ps1
gmake      → PATH 탐색 → C:\ti\ccs*\ccs\utils\bin\gmake.exe 폴백
```

### GUI 사용 순서

1. Spinbox에 원하는 Dead-time (ns, 100~400 범위) 입력
...
do_flash → powershell run_flash_node_8kw.ps1
→ (True, "완료") | (False, 사유)
```
...
- **실행**: `pip install pyserial matplotlib` 후 `python gui.py`
- **exe**: PyInstaller `8kw-gui.spec` → `tools/gui/dist/8kw-gui.exe` (~39 MB). `dist/`, `build/`는 .gitignore.
- **exe 경로 해결**: frozen 여부 `sys.frozen` 분기 → `sys.executable` 기준 4단계 위 = REPO_ROOT.
- ⚠️ exe는 재빌드 없이는 소스보다 구버전이 됨 — 소스 변경 후 배포 시 `pyinstaller 8kw-gui.spec` 재실행 필요.
...
**현황**: `tools/gui/dist/8kw-gui.exe` 존재 (~39 MB, PyInstaller). Python 직접 실행도 가능(pyserial·matplotlib).

**결정 필요**:

| 옵션 | 장점 | 단점 |
|------|------|------|
| (a) exe 배포 | Python 설치 불필요, 엔지니어 친화 | 소스 변경 시 재빌드 필요, 소스 drift 위험, 파일 크기 ~39 MB |
| (b) py 스크립트 배포 | 항상 최신, 크기 작음 | pip 의존성(pyserial·matplotlib) 설치 필요 |

exe 선택 시 추가로 정해야 할 것:
- 릴리즈 절차 (언제, 누가 `pyinstaller 8kw-gui.spec` 재실행)
- 버전 표기 (exe 내 `--version` 또는 창 타이틀에 git 해시/날짜)
- `dist/` 파일 git 추적 여부 (현재 .gitignore)
...
- **Physical 컬럼 placeholder** — 물리량 변환 계수 미입수 ([[adc_pinmap]] §미확인).
