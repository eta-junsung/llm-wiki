...
> ⚠️ **비자명 결합**: `gui.py:74` regex가 `ETA_BSP_PWM_DEADTIME_NS` 매크로를 직접 패칭한다. 이 매크로를 개명하거나 `eta_bsp_pwm.h`를 이동할 경우 **`gui.py:74`를 반드시 함께 갱신**해야 한다. 펌웨어 빌드 그린만으로는 이 회귀를 잡을 수 없다 — `gui.py --read` 라운드트립으로 확인할 것. 구조 배경 → [[firmware_layering]] §GUI↔펌웨어 비자명 결합.
