...
▶ Tx Module의 시스템 상태 / 운용 정보. **단, `Buffer[0]`(=Data[0]) 한 바이트는 두 성격이 섞여 있다**: bit0~4는 TX 보드 물리 상태(진짜 tx_status), **bit5·bit6은 통신 링크 heartbeat 상태로 TX 보드 상태가 아니다** (0x10에 함께 실릴 뿐). 그래서 코드 매크로도 bit5/6만 `COMM_ST_BIT_*` prefix로 분리. 상세 [[comm_state_monitoring]].
...
| `Buffer[0]` Bit.5 | `SPI_Comm_St` (심볼 `COMM_ST_BIT_SPI`) — 200ms 토글 heartbeat ([[comm_state_monitoring]]) |
| `Buffer[0]` Bit.6 | `BLE_Comm_St` (심볼 `COMM_ST_BIT_BLE`) — ESB 링크 활성 ([[comm_state_monitoring]]) |
