...
4. **구조적 결합 경우** — 배선 전부 정상인데 DOWN이면 03_TX_ble 미실행 또는 ESB 링크 미수립으로 valid 0x10 패킷이 02에서 나오지 않는 상태 — 위 "구조적 결합" 절 참조.
...
J-Link로 CPU를 halt시켜도 **RADIO 페리페럴은 계속 동작**한다. ESB HW auto-ACK가 CPU 없이 라디오 자동으로 나가므로 ESB 링크가 끊기지 않는다. "CPU 멈춤 ≠ RF 침묵".
