...
> **구현 현황 (2026-06-01)**: `03_TX_ble`의 `ble_link` 변수 항상 0 — ESB 수신 상태와 미연결. SPI 0x10 DATA[0] bit6은 죽은 필드(소비처 없음).  
> **후속 task (미착수)**: `esb_rx_cnt` 윈도우 기반 link 판정 후 `ble_link`에 대입. BLE 시절 "페어링" 개념에서 "ESB 수신 카운터 기반 링크 활성" 판단으로 재정의 예정. 사양 확정 후 본 페이지 갱신.
