...
### 릴레이 헤더 소유 원칙 — init seed

**(원칙)** raw-forward 릴레이 노드(02_RX_ble, 03_TX_ble)는 중계하는 패킷 **헤더를 소유**해야 한다. 데이터 출처(ESB 상대방)가 아직 연결되지 않아도, init 시 릴레이 버퍼를 **valid 헤더+LEN+CRC로 seed**해야 한다.

이유:
- **다운스트림 CRC 검증**: STM32가 수신 11B의 CRC를 검사해 불일치 패킷을 드롭한다. seed 없이 쓰레기 헤더가 흐르면 최초 ESB 데이터 도착 전까지 **모든 SPI 트랜잭션이 드롭**된다.
- **comm_st 운반**: heartbeat 비트(bit5)가 0x10 패킷에 얹혀 흐른다. valid 패킷이 없으면 comm_st가 끊겨 [[comm_state_monitoring]] "구조적 결합"이 발생한다.

seed는 실데이터가 수신되면 해당 슬롯이 덮여 **자기소멸** — 별도 해제 불필요.
