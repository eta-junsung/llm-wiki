...
## 4펌웨어 표준 패턴 (01 원형, 02·03 완료, 04 공유)

**01_RX_control이 원형, 02·03 적용 완료, 04는 01 골격 복사** — 02: `b92835c`(실보드 검증), 03: `1d7f71a`(빌드 ✓), 04: `07fbf1f`(01 복제→단순화, CubeIDE 빌드 미수행). **접두사**: 01/04(STM32)은 `app_*` 유지, 02/03(nRF52)은 nRF5 SDK 충돌 회피로 `eta_*` — [[nrf52_module_naming]].

### App 모듈 역할 네이밍 원칙 — `relay` vs `protocol`

| 서브프로젝트 | App 모듈명 | 역할 |
|---|---|---|
| 01_RX_control | `app_protocol` | **종단점** — 패킷 build/apply·ADC 센싱·UART 모니터 |
| 02_RX_esb (`eta_protocol`) | 중계 노드 ("relay") | SPI↔ESB 중계, seed, comm_st 판정 |
| 03_TX_esb (`eta_protocol`) | 중계 노드 ("relay") | ESB↔SPI 중계, ACK 조립, comm_st 판정 |
| 04_TX_control | `app_protocol` | **종단점** — 패킷 build/print·더미 TX, 01과 대칭 |

**교차프로젝트 일관성 = 리터럴 단어 아닌 패턴**: `eta_app_<역할>`. 01/04는 데이터 출처·소비 끝단이라 `protocol`, 02/03은 중계 노드라 사실상 `relay`다. `_shared/oled_tv_protocol`은 패킷 프로토콜 *정의* 자산이라 이름 그대로 유지.
...
### 02 esb_pkt seed 누락 — forward 버퍼 비대칭 버그 (2026-06-18 수정)

위 표는 **방향별 seed**를 기술하지만, TX→RX 방향에는 **두 개의 릴레이 버퍼**가 존재한다. 기존 표의 "03 seed" 커밋(`e72b86e`)은 03의 SPI→ESB forward 버퍼(spi_rx_pkt_by_hdr)만 커버했고, **02의 ESB→SPI forward 버퍼(esb_pkt)가 누락**됐다.

| 노드 | SPI수신→ESB forward | ESB수신→SPI forward |
|------|---------------------|---------------------|
| **02** | `spi_rx_pkt_by_hdr` 0x50~52 → seed됨 (`eta_protocol.c:222`, `6fc8b92`) | `esb_pkt` 0x10~12 → ★**seed 안 됨 (누락)** |
| **03** | `spi_rx_pkt_by_hdr` 0x10~12 → seed됨 (`:357`, `e72b86e`) | `g_last_ack_by_hdr` 0x50~52 → seed됨 (`:348`, `e72b86e`) |

**증상**: 01+02만 구동(03 off) 시 01이 SPI_Comm_St=DOWN 오판. 메커니즘: 03 down → `esb_pkt[0]=zero(hdr=0x00)` → 02가 bit5 stamp해도 01이 **hdr 게이트(`hdr==0x10&&len==0x08`, `app_protocol.c:107-119`)에서 드롭** → bit5 추출 불가 → SPI_FAIL. ([[comm_state_monitoring]] "구조적 결합 정밀 메커니즘")

**수정 (2026-06-18, 미커밋)**: 02 `protocol_init`에 `pkt_seed_buffers(esb_pkt, tx_hdrs, PKT_KIND_COUNT)` 추가 (`eta_protocol.c:227-232`). 03의 `:357/348` 거울. DK 보드 검증 완료(01+02 only, SPI UP/ESB DOWN). 커스텀 보드 사이클 닫을 때 commit.
...
### `_shared` 공개 API 표면 (`9ad338d` 기준, 네 펌웨어 공유)
...
| `pkt_print_status_line` / `pkt_print_data_line` | 사람용 텍스트 모니터 출력 (02/03 `Monitor_Loop` + 04 `print_packets`) |
