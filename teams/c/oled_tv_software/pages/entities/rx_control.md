...
- SYSCLK 64MHz — **HSI 소스** (회로도 8MHz HSE 수동 크리스탈 실장이나 실보드 HSERDY 100ms 타임아웃으로 발진 실패 → HSI로 전환. 상세 [[sysclk_hsi_transition]])
- 클럭 트리: HCLK 64 / APB1 32 / APB2 32 / 타이머클럭 64 / ADCCLK 8 MHz, Flash latency 2
- 빌드: STM32CubeIDE 2.1.1. float printf/scanf는 newlib-nano 플래그 선결 → [[cubeide_newlib_nano_float]].
...
> ⚠️ **정본 코드베이스 교체 (2026-06-15)**: 기존 dev 01(app_*.c 분산형·ESB/SPI 릴레이 중심)과 별개로, 팀원(Sean) 작성 전력제어 중심 코드베이스(AppSequence/AppCtrl/Soft MHz 구조, main.c 통합형 페리 init)가 **01 정본으로 채택**됨. dev의 ESB/SPI 릴레이 + UART 모니터(송신) 함수는 정본 위에 발췌 이식. 위 메인 루프 기술은 dev 기반이며 정본 기준 갱신 필요. STM32CubeIDE 2.1.1 빌드 통과·실보드 flash 완료(2026-06-15). UART 링크·ESB 실측 미완.
...
- **8.0 Mbps** (`BaudRatePrescaler=/4`, PCLK1=32 MHz — HSI 64MHz 기반 APB1). 프로토콜 매뉴얼 "9 Mbps 목표"는 미달 — 목표 유효성 미확정 ([[spi_link_reliability]] 미달 절, [[sysclk_hsi_transition]])
...
> ⚠️ **APB1 변경 주의 (HSI 전환 영향)**: PCLK1 36 → 32 MHz 변경으로 동일 prescaler/BS1/BS2 조합의 비트레이트가 달라짐. 설계 목표 비트레이트(미확인)를 PCLK1=32 MHz 기준으로 재계산·실측 확인 필요. → [[sysclk_hsi_transition]] "CAN 비트레이트"
