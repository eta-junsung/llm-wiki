...
`Core/Src/main.c:128-130`의 실제 `while(1)`은 **3개 호출**:

```c
while (1) {
    adc_proc();      // ADC 6채널 시퀀스 처리
    spi_proc();      // SPI 10ms 폴링 트랜잭션 (→ rx_to_tx / tx_to_rx 패킷)
    Monitor_Loop();  // UART 모니터 출력 (현재 주석처리 — 아래 주의)
}
```

- **UART 커맨드 파싱·실행은 이 루프에 없다** — 인터럽트(ISR) 컨텍스트에서 직접 처리되어 main loop를 선점한다. → [[uart_command_set#수신·파싱-메커니즘]]
- **현재 `Monitor_Loop()`는 주석처리로 비활성** (커밋 `175a8f7`, 03_TX_ble 모니터로 검증하느라 임시로 끔). 상태 추적은 [[status]].

> **문서 교정 후보**: 코드 repo의 `CLAUDE.md`는 main loop를 `LED_Loop / SPI_Loop / ESB_Loop / Monitor_Loop` 4종 polling으로 기술하나, 그 4종 패턴은 **nRF52 펌웨어(02_RX_ble / 03_TX_ble) 묘사**다. 01_RX_control의 실제 루프는 위 3개 호출 — **코드가 정본.**
