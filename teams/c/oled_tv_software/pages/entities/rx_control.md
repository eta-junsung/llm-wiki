...
while (1) {
    ...
    protocol_loop();   // SPI 송수신 + COMM 라인(edge) + 패킷 덤프(1초) (app_protocol)
}
```

- **`protocol_loop()`** = `app_protocol.c`의 공개 진입점. 내부 static `exchange_packets()`(수신 파싱 + 10ms 주기 송신) + `print_comm_line_on_change()`(COMM 라인, 링크 변화 edge에만, `2f2aa65`) + `print_packets()`(패킷 덤프 1초 `MONITOR_INTERVAL_MS`)로 분리 — **COMM 라인과 패킷 덤프의 주기가 다름**. 구 `spi_proc()`/`Monitor_Loop()`을 통합·대체. 적출·핸드오프 설계는 [[app_protocol_module]].
