...
| 04 | UI_Comm | CAN 트랜시버(ISOL1), UART→RS232(ISOL2+MAX232), 외부통신 커넥터(CON2). **UART 테스트 포인트: TP15=SCIB_RX, TP17=SCIB_TX** (COMM_P5V=5V 레벨) |
| 05 | Board_to_Board_Connector | PWM 출력, CN1(@Rx Power Board, PD3V3+신호), **CN2(@Rx Power Board, COMM_P5V)**, CN3·CN4(@BLE Module Board). Test Pins(TP19~TP25=GND/AGND) |
...
- ⚠ 페이지 05엔 보드투보드 헤더가 3개(CN1 = @Rx Power Board / **CN2 = @Rx Power Board (COMM_P5V 측)** / CN3·CN4 = @BLE Module Board). 페이지 02의 ST-LINK 커넥터 `CON1`과는 다른 부품이니 혼동 주의.

**커넥터 CN2 — Rx Power Board행 (COMM_P5V 진입점)** (회로도 p.5 vision 판독):

- 부품: `HEADER_2.0mm / 28P` (TMM-114-06-T-D-SM), 2.0 mm 피치 2열(14×2). 라벨 "연결@Rx OLED Regulator Power B/D" — CN1과 별도로 **절연 통신 전원**이 이 헤더로 들어온다.
- **핀 27·28 = COMM_P5V** (절연 5V), **핀 xx·xx = COMM_GND**. 이 보드에 COMM_P5V 생성 회로 없음 — 전적으로 CN2 경유 Power Board에서 공급.
- 디커플링: C44 (228M, 25V/3216) + C45 (104K, 50V/1608). LED5 (Green/2012) + R54 (2.2kΩ) — COMM_P5V 파워 인디케이터.
- **COMM_P5V = 5V** — 이 전원을 받는 신호(ISOL1·ISOL2 통신 측, MAX232, CAN 트랜시버 등)는 모두 5V 로직 레벨.
...
## UART 테스트 포인트 (p.4 UI_Comm)

| 레퍼런스 | 신호 | 전압 | 경로 | 방향 |
|---|---|---|---|---|
| **TP15** | SCIB_RX | **COMM_P5V (5V)** | ISOL2 B1 출력 → TP15 → MAX232 R1IN | 외부→MCU (수신) |
| **TP17** | SCIB_TX | **COMM_P5V (5V)** | ISOL2 B2 출력 → TP17 → MAX232 T1IN | MCU→외부 (송신) |

- 두 점 모두 디지털 절연기(ISOL2) **통과 후**, MAX232 RS232 변환 **이전** — UART 로직 레벨 신호.
- 전압이 **5V(COMM_P5V)**이므로 3.3V 로직 애널라이저 직접 연결 시 내압 주의.
- **MCU 3.3V UART 직접 검증**이 목적이면 ISOL2 MCU 측(A1/A2 핀) 또는 MCU 핀 직접(PC12=UART5_TX, PD2=UART5_RX) 프로브.

참고: 페이지 05 Test Pins(TP19~TP25)는 전부 GND/AGND — 신호 테스트 포인트 아님.
...
> **⚠️ 실보드 HSE 발진 실패 확인됨 (2026-06-15)**: 크리스탈 회로는 회로도 상 맞게 실장돼 있으나, 실보드에서 HSERDY 100ms 타임아웃으로 HSE 발진 실패. 원인은 크리스탈/부하 커패시터/레이아웃 HW 이슈로 추정. **현재 펌웨어는 HSI 내부 클럭으로 전환** — SYSCLK 64MHz(HSI/2 × PLL×16). 상세 [[sysclk_hsi_transition]].
