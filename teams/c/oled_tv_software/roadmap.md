...
- 코드 정리 라운드 (tasks/monitor-formatting 이후) — **코드 `9be1a7a`가 추월: 부분 구현**: ① 모니터 1-헤더-1-줄 압축 ✓(01), ② 공유 출력 함수(`_shared/oled_tv_protocol.{c,h}` 신설, `pkt_print_*`) ✓, ③ serialize/deserialize 통합(`pkt_build_*`/`pkt_apply_*`) ✓ 대체로, ④ ~~`SPI_PKT_*` 개명~~ 무효(이미 링크 중립 `PKT_HDR_*`). 추가 적출은 [[app_protocol_module]] 완료. ~~`_shared` 매크로 소유권 점검~~ → **✓ `9ad338d` 완료** (죽은 심볼 제거·매직넘버 상수화). 남은 일 = 02/03 실보드 검증·merge. → 작업 로드맵 [[roadmaps/spi-esb-refactor|SPI·ESB 리팩토링]].
