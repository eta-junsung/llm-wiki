...
- **남은 활성 작업은 M 호 밖의 별트랙뿐** — comm-state·monitor 정리 완료(`35b94d0`: UART monitor 바이너리화 + PC GUI [[pc_uart_gui]], 실보드 검증). **02 리팩토링·eta_ 전환 실보드 ✓(b92835c, 2026-06-11)**. **03_TX_ble 리팩토링 빌드 ✓(1d7f71a, 2026-06-11) — 실보드 △(TX 보드·오실로스코프 필요)**. 다음 = 02 ADD_SPI 점검·02/03 실보드 검증·SPI-down→bit5·N=20·`_shared` 레이어 다듬기(`oled_tv_protocol.{h,c}` 매크로 소유권 정리). 단일 소스 [[status]] "다음 시작점".
