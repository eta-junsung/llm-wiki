...
- `_shared/oled_tv_protocol.{h,c}`가 **네 펌웨어(01/02/03/04) 공유 단일 소스** — `pkt_build`/`apply`/`print`, 11B ESB 와이어 패킷. (와이어 사양 [[esb_packet_format]], [[spi_packet_format]], [[rx_to_tx_packets]], [[tx_to_rx_packets]])
...
→ R1~R3 부분 구현됨(공유 함수·링크 중립 상수), R4 무효. 01_RX_control은 [[app_protocol_module]] 적출로 빌드·동작확인 완료. **02_RX_ble 모듈 분리 리팩토링 빌드 완료(2026-06-10, 에러 0·경고 0) — 실보드 검증·미해결 항목(헤더명·ADD_SPI 전역) 결정 대기([[ses_build_conventions]])**. 03_TX_ble는 동형이나 미착수(build_tx_pkt/print_rx_pkt 비대칭 있음). 03 완료 후 merge·`_shared` 매크로 점검. 기능별 현황은 [[status]].
...
## 7. `_shared` 분할 특성화 — 미래 라운드 밑그림

`_shared/oled_tv_protocol.{h,c}`는 현재 5개 책임이 한 파일에 혼재한다:

| 책임 | 내용 | 소속 레이어 |
|------|------|-------------|
| **wire 계약** | `oled_tv_packet_t`·enum HDR·`PKT_*` 상수·비트위치·`_Static_assert`·BE inline | 공유 인터페이스 |
| **직렬화 ALG** | `pkt_build_*`·`pkt_apply_*`·`pkt_checksum`·`pkt_seed_buffers` | ALG (HW 무관, host에서 도는 순수 계산) |
| **printf 표시** | `pkt_print_status_line`·`pkt_print_data_line`·라벨 테이블 | App 보조 (stdout 의존) |
| **보드 핀** | `PIN_SPI_*`·`PIN_LED*` (02/03에서만 참조) | BSP |
| **정책 상수** | `PACKET_INTERVAL`·FW 버전·`COMM_ST_*` 윈도우 | App/프로젝트 정책 |

**누수 2건**:
1. ALG 직렬화 + printf가 한 `.c`에 묶여 `<stdio.h>`를 오염시킴 — ALG가 printf를 보면 host 단독 실행 불가.
2. 보드 핀(`PIN_SPI_*`·`PIN_LED*`)이 wire 계약 헤더에 혼입 — BSP 관심사가 와이어 형식 헤더로 누수.

**제약**:
- 4프로젝트(01/02/03/04) 공유. 01/04는 include-wrapper, 02/03은 SES `.emProject` 직접 참조
- wire 값(HDR·`PKT_*`·`oled_tv_packet_t`)은 host GUI `pc_uart_gui`(Python)가 미러링 → **불변**
- `<math.h>` 죽은 include (`.c`에 math 호출 0건) — 분할 시 제거

**계획된 접근 (우산-shim)**:
- `oled_tv_protocol.{h,c}`를 **우산(umbrella) shim**으로 유지
- 하위 파일로 분할: `wire.h`(wire 계약·불변) / `protocol_alg.{c,h}`(직렬화 ALG) / `monitor.{c,h}`(printf 표시)
- 01/03/04는 우산 shim include — **무편집**
- 핀(`PIN_SPI_*`·`PIN_LED*`) BSP 이전·식별자 `eta_` 개명은 각 프로젝트 레이어링 라운드로 미룸
- `PACKET_INTERVAL`→`SPI_PACKET_INTERVAL_MS` 분리는 §6 점검 결과와 연결

연결: [[status]] "레이어링 재구성" · [[firmware_layering]] §6 c팀 적용 현황
