...
**다음 시작점**: 01_RX_control 펌웨어 플래시 → UART5 물리 연결 → HW 엔지니어 [[pc_uart_gui]] 연결 검증. → [[roadmaps/uart-hw-debug]]

**예정 작업**:

1. **01 rx control UART 활성화 — HW 엔지니어 디버그** (신규·최우선): 01_RX_control 펌웨어 플래시 + UART5 물리 연결 + pc_uart_gui ADC/PWM 모니터링 검증. 배경: HW 팀이 제어 보드 통신 없이 수동 테스트 중 → 값 모니터링 불가. 상세 [[roadmaps/uart-hw-debug]].
2. **SPI 폴링 주기 10ms → 5ms** (01-02, 03-04 양쪽): `PACKET_INTERVAL`(또는 `SPI_PACKET_INTERVAL_MS`) 변경 후 01·04 양쪽 실보드 재검증.
3. **01·04 상호 SPI 링크 상태 인지**: 04-03 SPI 단절 시 01이 알도록, 01-02 SPI 단절 시 04가 알도록. 이를 위해 02·03이 자체적으로 SPI 통신 상태를 파악하는 메커니즘 필요 — 구체적인 구현 방향은 작업 진입 시 결정.
