...
source: conversation-2026-06-01 + 2026-06-15 프로브 CLI 실측(JLink ShowEmuList·pyocd·Get-PnpDevice) + 2026-07-01 g-8kw MSOX3104T CSV export Fs 방법론([[fod_i_coil_observation]])
date: 2026-07-01
---
...
**⚠️ CSV export로 오프라인 FFT 할 때 — Fs는 저장 점수(N)가 아니라 타임베이스가 정한다 (2026-07-01, g-8kw)**: Save/Recall → Save → CSV로 뽑을 때, 고정 Length(N)에서 **유효 Fs = N ÷ 창길이(T)** 이고 창길이(T = time/div × 10 div, 즉 **타임베이스**)가 Fs를 결정한다 — 점수(N)만 정해도 Fs는 안 정해진다. 화면 `Sa/s` 표시로 항상 재확인. 목적이 다르면(저주파 DC 레벨 vs 고주파 FFT) **타임베이스를 바꿔 별도로 두 번** 떠야 한다 — 하나의 창으로 둘 다 잡으려 하면 저주파용 긴 창은 고주파를 앨리어싱시킨다. 실사례·수치(85 kHz 대역 놓친 사례, 창 1 s·2000점 → Fs 2 kS/s로 앨리어싱)는 [[fod_i_coil_observation]] §2 CSV export 설정.
