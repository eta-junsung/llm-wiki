...
4. **지식 채널은 repo-native** — 코드 세션은 결정·함정·측정을 `docs/log.md`(+ `docs/<topic>.md`)에 남긴다. wiki를 언급하지 않는 보편적 엔지니어링 위생으로 표현하고, 지침의 집은 `eta-harness` coder/verifier 에이전트.
...
- **"log.md 남겨라"를 프로젝트별 CLAUDE.md에 박기** — 기각. 8kw엔 CLAUDE.md가 아예 없고, 생성해도 **모든 펌웨어 repo에 복붙**해야 함. 제네릭 규칙엔 잘못된 고도. → 공유 하네스(`eta-harness`)로.
- **`eta-firmware-conventions`(C 코딩 컨벤션 skill)에 넣기** — 기각. 그건 코드 스타일이고 **skill=온디맨드**라, "작업하며 로그 남겨라" 같은 **상시 습관**이 안정적으로 안 뜸. coder 에이전트 시스템 프롬프트(상시 활성)가 맞음.
...
- **감수**: 사람의 트리거 한마디("동기화해줘")는 남는다(내용 복붙은 아님). 코드 세션의 로깅 **규율**에 의존 — 로그도 커밋도 없이 지나간 경험은 wiki도 못 건짐(그래서 하네스 지침으로 강제). `docs/` 구조를 표준화(`docs/log.md`)하는 약한 결합을 받아들인다.
